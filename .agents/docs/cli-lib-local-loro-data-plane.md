...
`LocalLoroDataPlaneServer` engines (`@molly/shared`, owned by `LoroDocumentManager`).
...
- Electron main: `apps/electron/src/main/services/loro-data-plane-relay.ts` — a
  ...
  navigated windows.
- Renderer adapter: `@molly/shared` `local-loro-transport.ts` — filters inbound frames
...
## Join recovery and history

Join attempts have a 120-second deadline. Health checks never extend it; first-sync
success requires reconciliation in the same connection generation. Disconnect, close
and rejoin invalidate old completions. Failure remains recoverable. Regression:
`packages/shared/tests/local-loro-join-recovery.test.ts`.

Transport still carries one Loro document. The control-plane Mirror ignores history;
`sessionData.history` provides directory, window and explicit whole-history reads.
One validated HistoryWriter owns mutations; no second persistent store is introduced.
