...
`sessions/session-list-rows.ts`, `lib/session-opened-by-tree.ts`). Binding rules live in
[that directory's AGENTS.md](../../packages/components/src/components/AGENTS.md); this page
...
example through the `molly_session_create` MCP tool. It is presentation only: opened
