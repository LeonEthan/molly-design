...
Internal map for `@molly/loro-streams-rpc`. The package `README.md` is the npm
...
Historical workspace/machine-level JSON RPC over Loro Streams. The local
Molly desktop uses Electron Machine RPC for Code Collab and file
previews; its renderer does not create a Streams client.
...
- The local renderer calls Electron IPC through
  `packages/components/src/providers/workspace-machine-rpc-facade.ts`.
- The local CLI serves Code Collab v2 through
  `apps/cli/src/lib/code-collab/code-collab-v2-service.ts`.
- The Streams schemas remain for historical protocol readback. They are not
  composed by the local desktop runtime.
