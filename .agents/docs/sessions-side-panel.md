...
- `session-detail.tsx` — outer shell/tabs (desktop: `desktop-session-detail-layout.tsx`;
  ...
  another (the `molly_session_create` MCP tool). That
  ...
  The header "More" menu forks the latest completed rendered assistant turn through the
  ...
  Human fork actions create a local Tab or Side Chat without a Git
  ...
  Browser side-panel state is named `browser`. Once opened, keep `SessionBrowserPanel` mounted while other fixed side-panel
  ...
  Panel mount is not preview ownership: never release a local endpoint
  ...
  `clearManagedPreviewFrame` calls at each release site. On a cache miss,
  reacquire the idempotent local endpoint.
...
Molly P1 adds the closeable `design` fixed tab for Sessions with an artwork
