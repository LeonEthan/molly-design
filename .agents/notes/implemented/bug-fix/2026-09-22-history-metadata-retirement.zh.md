...
[剩余内存增长复核](../testing/2026-09-22-memory-growth-attribution.zh.md)随后取得类型与生命周期证据：独立空白窗口的 BRP 保留来自 30 秒 keepalive 延迟；实际 Work 的逐次小项则来自每个存储上下文有淘汰边界的 localStorage 区域缓存，二者不能混为同一持有链。macOS 私有内存还受到共享内存扣除口径和 V8 保留容量影响。本记录保留当时的候选与原始数值；“尚未确定具体持有者”不再代表这些已完成对照的当前状态。新证据仍不逐字节解释本轮标准 Work 的全部增量。Issue 随后按维护者决定阶段性关闭，剩余优化保留在[关闭 comment](https://github.com/LeonEthan/molly-design/issues/45#issuecomment-5773885518)，不代表全部增长已消除。
