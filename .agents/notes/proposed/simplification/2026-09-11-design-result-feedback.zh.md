...
| 27、28：开发功能退出与通用能力 | 终端（`components/terminal/terminal-dock.tsx`，已删除）、[浏览器](../../../../packages/components/src/components/sessions/session-browser-panel.tsx)、[fork](../../../../apps/cli/src/session/session-fork-service.ts)、文件读取均已有 | 仍有消费者则复用；退出开发 UI 不等于删除 Agent 读图/文件/工具或设计预览依赖 |
