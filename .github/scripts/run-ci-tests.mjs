...
export function runCiTests({
  ...
  const nonElectron = testPackages.filter((name) => name !== '@molly/electron');
  ...
  if (testPackages.includes('@molly/electron')) {
    runPnpm(execFileSync, cwd, ['--filter', '@molly/electron', 'run', 'test']);
  }
  if (nonElectron.length === 0 && !testPackages.includes('@molly/electron')) {
    runPnpm(execFileSync, cwd, ['test:ci']);
    return 'full';
  }
  return 'affected';
