...
**Status:** implemented-awaiting-native-qa
...
The targeted regression includes five new mounted interruption tests with virtual time, controlled media and explicit native/DOM foreground signals. Electron `build:app` passes. The old build's native minimize defect was reproduced with the real MP3 player; post-fix native acceptance is pending because the test Mac is locked and cannot foreground its isolated Electron window. Installed-desktop and in-app hearing acceptance remain pending.
