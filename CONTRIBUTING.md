...
By default, the open-source desktop app stores data in `~/.lody-oss`. To avoid using existing data during development, set `MOLLY_DATA_DIR`:

```bash
MOLLY_DATA_DIR="$(pwd)/.lody-dev-data" pnpm start:local
```

PowerShell:

```powershell
$env:MOLLY_DATA_DIR = "$PWD/.lody-dev-data"
pnpm start:local
```
