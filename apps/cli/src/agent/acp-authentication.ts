...
import { withMollyNpmCacheForNpx } from './npx-cache';
import { withAcpSessionStartSlot } from './acp-session-start-gate';
import { withLoopbackNoProxy } from '@molly/shared/proxy-env';
import {
  mergeACPProcessEnv,
  mergeLoginShellEnv,
...
function toAuthenticationForm(
  ...
  for (const [id, rawProperty] of propertyEntries) {
    ...
    const secret =
      getMollyElicitationMeta(property._meta)?.secret === true ||
      meta?.secret === true ||
      meta?.sensitive === true ||
        ...
        await runNpxStartupWithRecovery({
          ...
          attempt: async ({ args }) => {
            ...
            try {
              ...
              const app = acp
                .client({ name: 'molly-authentication' })
                .onRequest(acp.methods.client.elicitation.create, async ({ params }) => {
                  ...
                  if (raw?.mode === 'url') {
                    if (typeof raw.url !== 'string' || !isAllowedAuthorizationUrl(raw.url)) {
                      const error = `${options.agentType} requested an unsafe authentication URL. Molly only opens HTTP and HTTPS authorization pages.`;
                  ...
                  if (!form) {
                    const error = `${options.agentType} requested an unsupported authentication form or one larger than Molly's ${ACP_AUTHENTICATION_FORM_MAX_BYTES / 1024} KiB limit. Molly currently supports text, secret, and single-select fields.`;
                    interactionError = error;
                    this.logger.debug(`[acp-auth] ${error}`);
                    options.onProgress?.({ status: 'error', error });
                    return { action: 'decline' as const };
                ...
                app.connectWith(stream, async (context) => {
                  const initialized = await context.request(
                    acp.methods.agent.initialize,
                    {
                      ...
                      clientInfo: { name: 'molly', title: 'Molly', version: '1' },
                    },
