...
type AcpSessionCapabilitiesResponse = {
  ...
  /** Session-response meta; adapters publish Molly-owned contracts here. */
  _meta?: Record<string, unknown> | null;
};

const zMollySessionResponseMeta = z.object({
  _meta: z
    .object({
      lody: z
        .object({
...
/**
 * Per-model reasoning-effort ladders an adapter published under the Molly-owned
 ...
 */
export function readMollyModelReasoningEfforts(
  sessionResponse: unknown
): Record<string, string[]> | undefined {
  const parsed = zMollySessionResponseMeta.safeParse(sessionResponse);
...
export function normalizeAcpSessionCapabilities(
  ...
  // Molly "_meta.lody" map the adapter translates vendor metadata into (Grok).
  const modelReasoningEfforts = {
    ...readMollyModelReasoningEfforts(sessionResponse),
