import { ACP_COLD_NPX_INIT_TIMEOUT_MS, ACP_NPX_STARTUP_MAX_ATTEMPTS } from '@molly/shared';
...
/**
 * Re-exported so the client-side backstop in `@molly/shared/acp-startup-budget`
      ...
      if (shouldPurgeMollyNpmCache) {
        ...
        const lodyPurged =
          brokenPurged.length === 0
            ? purgeMollyNpmCache({ env: options.env, io: options.npxCacheIo })
            : [];
        await options.cleanupFailedAttempt?.();
        options.logger.warn(
          `${options.logPrefix} ${npxSpec.name}@${npxSpec.version} failed during npx startup; ` +
            `purged ${brokenPurged.length + lodyPurged.length} Molly npm cache dir(s), retrying ` +
