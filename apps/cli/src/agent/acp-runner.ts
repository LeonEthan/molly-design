...
import { ACP_STARTUP_QUEUE_WAIT_TIMEOUT_MS } from '@molly/shared/acp-startup-budget';
...
function prepareCodexHomeEnv(
  ...
  const isTitleAgentCodexRun =
    isBuiltinCodex && (baseEnv.MOLLY_TITLE_AGENT ?? baseEnv.LODY_TITLE_AGENT) === '1';
  const shouldUseWorkdirCodexHome =
    isBuiltinCodex &&
    !baseEnv.CODEX_HOME &&
    ((baseEnv.MOLLY_E2E ?? baseEnv.LODY_E2E) === '1' || isTitleAgentCodexRun);
...
export const startLocalAcpAgent = async (options: StartLocalAcpAgentOptions) => {
  ...
  // shell is covered too: the agent reaches Molly's MCP HTTP host over
  ...
  const keepCodexHome = (env.MOLLY_KEEP_CODEX_HOME ?? env.LODY_KEEP_CODEX_HOME) === '1';
