  ...
  constructor(
    ...
    super(
      `DeepSeek Harness session root ${JSON.stringify(
        ...
        zstdArtifact
      )}). Molly left all session artifacts unchanged. Back up the root and migrate every session to one compression mode, or use a separate DSH_HOME before retrying.`
    );
 ...
 * Molly-only raw root keeps `none` so an upgrade never strands its sessions.
 */
...
async function publishConfigAtomically(configPath: string, config: string): Promise<void> {
  ...
  try {
    ...
    // On Windows rename cannot replace an existing destination. Another Molly
