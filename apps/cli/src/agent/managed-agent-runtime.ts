...
import { resolveRuntimeArtifactUrl } from '@molly/platform';

const COMPLETE_MARKER = '.molly-complete';
...
export class ManagedAgentRuntimeManager {
  ...
  constructor(options: ManagedAgentRuntimeManagerOptions = {}) {
    this.rootDir = options.rootDir ?? join(getMollyDataDir(), 'agent-binaries');
  ...
  private async readInstallation(
    ...
    if (!(existsSync(join(dir, COMPLETE_MARKER)) || existsSync(join(dir, '.lody-complete')))) {
      return null;
    }
  ...
  private async downloadAndPublish(
    ...
    try {
      ...
      if (
        signal.aborted &&
        !(existsSync(join(dir, COMPLETE_MARKER)) || existsSync(join(dir, '.lody-complete'))) &&
  ...
  private async publish(unpackDir: string, dir: string): Promise<void> {
    ...
    try {
      ...
      if (existsSync(join(dir, COMPLETE_MARKER)) || existsSync(join(dir, '.lody-complete'))) return;
      throw error;
    }
  }
}
