...
import {
  ...
  purgeMollyNpmCache,
  withMollyNpmCacheForNpx,
} from './npx-cache';

const originalPlatform = process.env.MOLLY_PLATFORM;
const originalDataDir = process.env.MOLLY_DATA_DIR;
...
afterEach(() => {
  if (originalPlatform === undefined) delete process.env.MOLLY_PLATFORM;
  else process.env.MOLLY_PLATFORM = originalPlatform;
  if (originalDataDir === undefined) delete process.env.MOLLY_DATA_DIR;
  else process.env.MOLLY_DATA_DIR = originalDataDir;
});
...
describe('runNpxStartupWithRecovery', () => {
  ...
  it('purges Molly npm cache and retries after a cold connection.initialize timeout', async () => {
    const attempts: NpxStartupAttemptInput[] = [];
    const cache = getMollyNpmCacheDir();
...
describe('withLodyNpmCacheForNpx', () => {
  it('isolates npx-launched ACP agents onto the Molly npm cache', () => {
    const env = {
      PATH: '/usr/bin',
      npm_config_cache: '/Users/u/.npm',
      NPM_CONFIG_CACHE: '/Users/u/.npm-uppercase',
    };
    const next = withMollyNpmCacheForNpx('npx', env, { home: '/Users/u' });
    const expectedCache = getMollyNpmCacheDir('/Users/u');
  ...
  it('leaves non-npx agents untouched', () => {
    ...
    expect(withMollyNpmCacheForNpx('node', env, { home: '/Users/u' })).toBe(env);
  });
});
...
describe('npm cache isolation helpers', () => {
  ...
  it('recognizes only the Molly npm cache path', () => {
    expect(isMollyNpmCacheDir('/Users/u/.molly/npm-cache', '/Users/u')).toBe(true);
    expect(isMollyNpmCacheDir('/Users/u/.molly/npm-cache/', '/Users/u')).toBe(true);
    expect(isMollyNpmCacheDir('/Users/u/.geon/npm-cache', '/Users/u')).toBe(false);
    expect(isMollyNpmCacheDir('/Users/u/.lody/npm-cache', '/Users/u')).toBe(false);
    expect(isMollyNpmCacheDir('/Users/u/.lody-oss/npm-cache', '/Users/u')).toBe(false);
    expect(isMollyNpmCacheDir('/Users/u/.npm', '/Users/u')).toBe(false);
    expect(isMollyNpmCacheDir(undefined, '/Users/u')).toBe(false);
  });

  it('recognizes the cloud installation cache only in cloud mode', () => {
    process.env.MOLLY_PLATFORM = 'cloud';
    expect(isMollyNpmCacheDir('/Users/u/.lody/npm-cache', '/Users/u')).toBe(true);
    expect(isMollyNpmCacheDir('/Users/u/.molly/npm-cache', '/Users/u')).toBe(false);
  });
});
...
describe('purgeLodyNpmCache', () => {
  it('purges only Molly-owned _npx and _cacache dirs', () => {
    ...
    const purged = purgeMollyNpmCache({
      env: { npm_config_cache: cache },
      home: '/Users/u',
      io,
    });
