...
export function getMollyNpmCacheDir(home: string = homedir()): string {
  return join(getMollyDataDir(undefined, home), 'npm-cache');
}
...
export function isMollyNpmCacheDir(
  ...
  return (
    ...
    normalizeCachePathForCompare(getMollyNpmCacheDir(home))
  );
}
 ...
 * then prevents ACP from starting before Molly can show a useful agent response.
 * Keep only npx-launched ACP adapters on a Molly-owned cache; local binaries and
 * non-npx custom agents keep their normal environment.
 */
export function withMollyNpmCacheForNpx(
  command: string,
  ...
  const cacheDir = getMollyNpmCacheDir(options.home);
  return {
    ...env,
...
export interface PurgeMollyNpmCacheArgs extends NpxCacheLocateOptions {
  io?: Pick<NpxCacheIo, 'existsSync' | 'rmSync'>;
}

/**
 * Remove only Molly-owned npm cache state after npm itself reports cache
 ...
 * manager state, while the active Molly installation's `npm-cache` exists solely
 * for ACP npx adapters and can be rebuilt safely on the next retry.
 */
export function purgeMollyNpmCache(args: PurgeMollyNpmCacheArgs): string[] {
  const io = args.io ?? defaultIo;
  const cacheDir = getConfiguredNpmCacheDir(args.env ?? process.env);
  if (!cacheDir || !isMollyNpmCacheDir(cacheDir, args.home)) {
    return [];
  }
