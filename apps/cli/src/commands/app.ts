...
export const appCommand = new Command('app')
  .description(
    'Open the Molly desktop app on a new chat with a local directory selected (registers it if needed)'
  )
  ...
  .action(async (projectPath: string, options: AppCommandOptions) => {
    await runOneShotCommand('app', options, async () => {
      ...
      try {
        ...
        throw new Error(
          `Could not hand the link to the operating system. Is the Molly desktop app installed? Open it manually: ${deepLink}`,
          { cause: error }
        );
      }
      ...
      logger.success(`✅ Opening Molly desktop: ${target.name} (${rootPath})`);
    });
  });
