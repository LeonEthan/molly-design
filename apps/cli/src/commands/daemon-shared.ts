...
import { requestLocalCliHostShutdown } from '@molly/shared/node/local-cli-host-lease';
import { calculateWorkerMaxOldSpaceMiB } from '@molly/cli-supervisor';
export { MOLLY_LOG_DIR } from '@/utils/log-retention';

export const DAEMON_PID_FILE = path.join(getMollyDataDir(), 'daemon.pid');
...
// plus the private Host-control token `molly daemon stop` authenticates with.
...
export function reportDaemonRunnerLaunchOutcome(outcome: DaemonRunnerLaunchOutcome): void {
  const fdRaw = process.env[DAEMON_RUNNER_READY_FD_ENV] ?? process.env.LODY_DAEMON_RUNNER_READY_FD;
  delete process.env[DAEMON_RUNNER_READY_FD_ENV];
  delete process.env.LODY_DAEMON_RUNNER_READY_FD;
  if (!fdRaw) return;
