...
it('encodes edit files as real multipart form data without exposing the key in the body', async () => {
  ...
  vi.stubGlobal('fetch', async (url: string, init: RequestInit) => {
    ...
    expect(wire.headers.get('authorization')).toBe(`Bearer ${SECRET_KEY}`);
    ...
    expect([...form.values()].filter((value) => typeof value === 'string')).not.toContain(
      SECRET_KEY
    );
  ...
  try {
    const result = await fetchImageHttpTransport({
      url: 'https://images.example/v1/images/edits',
      method: 'POST',
      headers: { authorization: `Bearer ${SECRET_KEY}` },
      multipart: {
        fields: { model: 'saved-model', prompt: 'edit it', n: '1' },
        files: [
          { field: 'image[]', filename: 'source.png', mimeType: 'image/png', bytes: png },
          { field: 'mask', filename: 'mask.png', mimeType: 'image/png', bytes: png },
        ],
      },
      timeoutMs: 1000,
      maxBytes: 1024,
