...
it('accepts old bootstrap credentials but removes both generations before child launch', () => {
  const env = {
    LODY_ELECTRON_SESSION_TOKEN: 'old-token',
    LODY_ELECTRON_SESSION_USER_ID: 'old-user',
    MOLLY_ELECTRON_SESSION_TOKEN: 'new-token',
    PATH: '/bin',
  };
  expect(withoutElectronBootstrapCredentials(env)).toEqual({ PATH: '/bin' });
  expect(consumeElectronBootstrapCredentials(env)).toEqual({ sessionToken: 'new-token', sessionUserId: 'old-user' });
  expect(env).toEqual({ PATH: '/bin' });
});
