import { parseHistoryWrite, ToolCallMessageSchema, parseMollyTaskMeta } from '@molly/shared';
...
import type { SessionPlanEntry } from '@molly/shared';
  ...
  try {
    if (persistableBatch.length > 0) {
      ...
      if (!targetTurnId && callbacks?.allowAutonomousAssistantEntry !== true) {
        ...
        const targetOnly =
          targetTurnId &&
          persistableBatch.every((notification) => {
        ...
        await doc.agentWrites.applyAgentBatch({
          ...
          createId: targetTurnId ? () => targetTurnId : uuidV4,
          ...(model ? { model } : {}),
        });
      }
    }
...
const toolUpdateWriteSchema = ToolCallMessageSchema.partial();
...
const validateNotificationForHistory = (
  ...
  switch (update.sessionUpdate) {
    ...
    case 'tool_call_update':
      ...
      try {
        parseHistoryWrite(toolUpdateWriteSchema, update);
      } catch {
        return { ok: false, reason: 'invalid_tool_update' };
      }
      return { ok: true };
    case 'plan':
...
const filterNotificationsForHistory = (
  ...
  const shouldKeep = (message: AcpSessionNotification): boolean => {
    ...
    if (parseMollyTaskMeta(update._meta)) return true;
