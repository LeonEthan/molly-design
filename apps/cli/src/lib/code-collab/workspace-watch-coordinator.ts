...
const MINIMAL_ENV_NAMES = [
  ...
  'LANG',
  'LC_ALL',
  // The packaged Electron CLI runs through Molly Helper as a Node runtime.
...
function readWorkspaceWatchMode(): 'broker' | 'off' {
  return (process.env.MOLLY_CODE_COLLAB_WATCH_MODE ?? process.env.LODY_CODE_COLLAB_WATCH_MODE) === 'off' ? 'off' : 'broker';
}
