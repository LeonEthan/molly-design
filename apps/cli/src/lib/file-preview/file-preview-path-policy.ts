 ...
 * the OS temp directory and Molly's own chat working directories. Everything else
 ...
 *
 * `MOLLY_FILE_PREVIEW_EXTRA_ROOTS` (platform path-delimiter separated) is the one
 * explicit opt-in for widening this on a machine the operator controls.
 */
export const FILE_PREVIEW_EXTRA_ROOTS_ENV_VAR = 'MOLLY_FILE_PREVIEW_EXTRA_ROOTS';
...
export function getDefaultFilePreviewExtraRoots(
  ...
  const configured = ((env[FILE_PREVIEW_EXTRA_ROOTS_ENV_VAR] ?? env.LODY_FILE_PREVIEW_EXTRA_ROOTS) ?? '')
  ...
  return [
    ...
    path.join(getMollyDataDir(undefined, options.homeDir), 'chats'),
    ...configured,
  ];
}
...
export function resolveFilePreviewPath(args: {
  ...
  const notAllowed: FilePreviewPathResolution = {
    ...
    rejection: {
      ...
      message:
        'File is outside the workspace: preview is limited to this session’s workspace and Molly temporary directories.',
    },
  };
