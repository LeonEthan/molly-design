...
import { MOLLY_MANAGED_GH_TOKEN_SHA256_ENV } from '@/lib/gh-token-injector';
...
const REAL_GH_PATH_PLACEHOLDER = '__MOLLY_REAL_GH_PATH__';
const NODE_EXEC_PATH_PLACEHOLDER = '__MOLLY_NODE_EXEC_PATH__';
const MANAGED_TOKEN_MARKER_ENV = MOLLY_MANAGED_GH_TOKEN_SHA256_ENV;
...
const readRepoFullName = async () => {
  const fromEnv =
    (process.env.MOLLY_GITHUB_REPO_FULL_NAME ?? process.env.LODY_GITHUB_REPO_FULL_NAME) ||
...
const BROKER_STATE_PATHS = [
  ...((process.env.MOLLY_PLATFORM ?? process.env.LODY_PLATFORM) === 'cloud' ? ['/home/node/.lody/broker.json'] : []),
  path.join(
...
export const getGhShimHostBinDir = (): string => path.join(getMollyDataDir(), 'bin');
