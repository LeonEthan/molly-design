...
const debugEnabled = isTruthyEnv((process.env.MOLLY_GIT_CRED_HELPER_DEBUG ?? process.env.LODY_GIT_CRED_HELPER_DEBUG));
const debugFile = (process.env.MOLLY_GIT_CRED_HELPER_DEBUG_FILE ?? process.env.LODY_GIT_CRED_HELPER_DEBUG_FILE);
...
// MOLLY_GIT_CRED_BROKER_STATE_FILE, when set, names the state file of the broker
...
const BROKER_STATE_PATHS = [
  (process.env.MOLLY_GIT_CRED_BROKER_STATE_FILE ?? process.env.LODY_GIT_CRED_BROKER_STATE_FILE),
  ...((process.env.MOLLY_PLATFORM ?? process.env.LODY_PLATFORM) === 'cloud' ? ['/home/node/.lody/broker.json'] : []),  // Container path
  path.join(
    (process.env.MOLLY_DATA_DIR ?? process.env.LODY_DATA_DIR) ||
      path.join(os.homedir(), (process.env.MOLLY_PLATFORM ?? process.env.LODY_PLATFORM) === 'cloud' ? '.lody' : '.molly'),
    'broker.json'
 ...
 * 2. The CLI sets MOLLY_GIT_CRED_BROKER_URL to a Docker-reachable address
...
const getBrokerConfig = () => {
  ...
  const envUrl = (process.env.MOLLY_GIT_CRED_BROKER_URL ?? process.env.LODY_GIT_CRED_BROKER_URL);
  const envToken = (process.env.MOLLY_GIT_CRED_BROKER_TOKEN ?? process.env.LODY_GIT_CRED_BROKER_TOKEN);
  if (envUrl && envToken) {
...
const getContextToken = () => {
  const value = (process.env.MOLLY_GIT_CRED_CONTEXT_TOKEN ?? process.env.LODY_GIT_CRED_CONTEXT_TOKEN);
...
export const getCredentialHelperHostPath = (repoId: RepoId): string =>
  path.join(getMollyDataDir(), 'repos', repoId, HELPER_BASENAME);
