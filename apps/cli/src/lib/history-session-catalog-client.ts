...
function getMollyReadSessionHistoryMethod(initResponse: acp.InitializeResponse): string | null {
...
export async function requestHistorySessionReplay(args: {
  ...
  if (args.provider.cliType === 'builtin' && args.provider.agentType === 'codex') {
    const method = getMollyReadSessionHistoryMethod(args.initResponse);
    if (!method) {
      throw new Error(
