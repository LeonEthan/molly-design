...
export { decideHistoryRefresh, decideHistoryConflictResolution } from '@molly/shared/session-data';
export type {
...
function resolveSessionTitle(info: SessionInfo, provider: LocalProjectHistoryProvider): string {
  ...
  // which can carry Molly-appended instruction tails.
  const cleaned = info.title?.trim() ? sanitizeMollyInternalInstructions(info.title) : '';
