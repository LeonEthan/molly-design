...
import type { MachineLegacyMetaFields } from '@molly/shared';
...
export type ResolveLocalProjectRootPathRetryOptions = {
  ...
  /** Total reads including the first. Default: MOLLY_LOCAL_PROJECT_RESOLVE_MAX_ATTEMPTS or 4. */
  maxAttempts?: number;
  /** Wait between attempts. Default: MOLLY_LOCAL_PROJECT_RESOLVE_RETRY_DELAY_MS or 400. */
  retryDelayMs?: number;
  /**
   ...
   * MOLLY_LOCAL_PROJECT_RESOLVE_SYNC_TIMEOUT_MS or 1500.
   */
...
export async function resolveWorkspaceLocalProjectWithSyncOnMiss(
  ...
  const syncTimeoutMs =
    options.syncTimeoutMs ?? readTimeoutEnv('MOLLY_LOCAL_PROJECT_RESOLVE_SYNC_TIMEOUT_MS', 1_500);
  try {
    await withTimeout(
      options.requestSync(),
      syncTimeoutMs,
...
export async function resolveWorkspaceLocalProjectRootPathWithRetry(
  ...
  const maxAttempts = Math.max(
    1,
    options.maxAttempts ?? readTimeoutEnv('MOLLY_LOCAL_PROJECT_RESOLVE_MAX_ATTEMPTS', 4)
  );
  const retryDelayMs =
    options.retryDelayMs ?? readTimeoutEnv('MOLLY_LOCAL_PROJECT_RESOLVE_RETRY_DELAY_MS', 400);
  const syncTimeoutMs =
    options.syncTimeoutMs ?? readTimeoutEnv('MOLLY_LOCAL_PROJECT_RESOLVE_SYNC_TIMEOUT_MS', 1_500);
