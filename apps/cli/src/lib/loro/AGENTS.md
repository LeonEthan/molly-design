...
Every synced-doc Mirror uses `ignoreUnknownProperties: true`: unknown peer roots must
not block writes. Regression: `packages/shared/tests/session-doc-forward-compat.test.ts`.

Session Mirrors hold only the control plane and exclude history. All history writes use
`HistoryWriter` through `sessionData`/`agentWrites`: validate new and changed fields,
preserve unchanged unknown legacy data. Reads use the directory or targeted reader;
...
session retains that subscription until `cleanSessionDoc`. Opening is not a cheap read.
...
fan-out per `MOLLY_LORO_META_SYNCED_MIN_INTERVAL_MS` (30s), deferred and never dropped,
...
not survive `MOLLY_LORO_HEALTH_STABILITY_WINDOW_MS` (5s) counts as a failed recovery and
