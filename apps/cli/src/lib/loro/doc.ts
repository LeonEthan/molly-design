...
import { createLocalLoroDataPlaneScheduler } from '@molly/shared/local-loro-data-plane-scheduler';
...
import type { CloudBillingPort, CloudStreamsTokenPort } from '@molly/platform';
...
export class LoroDocumentManager {
  ...
  static async create(
    ...
    const createRepoTimeoutMs = readTimeoutEnv('MOLLY_LORO_CREATE_REPO_TIMEOUT_MS', 30_000);
    const joinMetaTimeoutMs = readTimeoutEnv('MOLLY_LORO_JOIN_META_TIMEOUT_MS', 30_000);
    ...
    if (options.attachRemoteOnCreate === true) {
      ...
      try {
        ...
        const syncMetaTimeoutMs = readTimeoutEnv('MOLLY_LORO_SYNC_META_TIMEOUT_MS', 20_000);
  ...
  private readonly remoteSyncPersist = new PersistCoalescer<LoroRepoPersistReason>({
    debounceMs: readTimeoutEnv('MOLLY_LORO_REMOTE_PERSIST_DEBOUNCE_MS', 200),
    flush: async (reasons) => {
  ...
  async syncMetaOrThrow(options: { timeoutMs?: number; reason?: string } = {}): Promise<void> {
    ...
    const timeoutMs =
      options.timeoutMs ?? readTimeoutEnv('MOLLY_LORO_SYNC_META_TIMEOUT_MS', 20_000);
  ...
  async syncDocOrThrow(
    ...
    const reason = options.reason ?? 'explicit-sync';
    const timeoutMs = options.timeoutMs ?? readTimeoutEnv('MOLLY_LORO_SYNC_DOC_TIMEOUT_MS', 8_000);
  ...
  async syncFlockDocOrThrow(
    ...
    const reason = options.reason ?? 'explicit-sync';
    const timeoutMs =
      options.timeoutMs ?? readTimeoutEnv('MOLLY_LORO_SYNC_MACHINE_FLOCK_TIMEOUT_MS', 8_000);
  ...
  private async destroyRepo(options: { fast?: boolean }): Promise<void> {
    ...
    const destroyTimeoutMs = readTimeoutEnv('MOLLY_LORO_DESTROY_TIMEOUT_MS', 1_000);
   ...
   */
  getPresenceStates(): MollyPresenceStateMap | null {
   ...
   */
  subscribePresenceStates(listener: (states: MollyPresenceStateMap) => void): (() => void) | null {
  ...
  configureMachineMonitor(
    machineId: MachineId,
    snapshotProvider: () => Promise<import('@molly/shared').MachineMonitorSnapshot>
  ): void {
...
export class SessionDocument implements LoroDocument<Omit<SessionDocMeta, 'history'>, SessionMeta> {
  ...
  private async startDocRoomSync(): Promise<void> {
    ...
    for (let attempt = 0; attempt < MAX_JOIN_RETRIES; attempt++) {
      ...
      try {
        ...
        const syncDocTimeoutMs = readTimeoutEnv('MOLLY_LORO_SYNC_DOC_TIMEOUT_MS', 8_000);
  ...
  async waitUntilSynced(options: { timeoutMs?: number } = {}): Promise<boolean> {
    ...
    const timeoutMs =
      options.timeoutMs ?? readTimeoutEnv('MOLLY_LORO_WAIT_DOC_SYNC_TIMEOUT_MS', 4_000);
