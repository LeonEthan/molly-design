...
import {
  ...
  MOLLY_MACHINE_MONITOR_MACOS_SAMPLE_MS,
  MOLLY_MACHINE_MONITOR_UNIX_SAMPLE_MS,
  MOLLY_MACHINE_MONITOR_WINDOWS_SAMPLE_MS,
  ...
  toMollyMachineMonitorStreamUrl,
  type MachineId,
...
export class CliMachineMonitorRuntime {
  private readonly store = new EphemeralStore(MOLLY_MACHINE_MONITOR_STATE_TTL_MS);
  ...
  attachStreams(streamsOptions: CliMachineMonitorStreamsOptions): void {
    ...
    this.transport = new EphemeralStreamCrdt({
      streamUrl: toMollyMachineMonitorStreamUrl(durableStreamUrl),
