...
const REMOTE_INSTANCE_ID = 'remote-instance' as MollyPresenceInstanceId;
...
const replicatePeerFromPresenceRoom = (presence: CliPresenceRuntime): void => {
  const peer = new EphemeralStore(MOLLY_PRESENCE_TTL_MS);
  peer.set(getMollySessionPresenceKey(REMOTE_SESSION_ID, REMOTE_INSTANCE_ID), {
    kind: 'session',
...
const decodeLocalOriginSnapshot = (presence: CliPresenceRuntime): MollyPresenceStateMap => {
  const decoded = new EphemeralStore(MOLLY_PRESENCE_TTL_MS);
  try {
    decoded.apply(presence.encodeLocalOriginPresence());
    return parseMollyPresenceStates(decoded.getAllStates() as Record<string, unknown>);
  } finally {
    decoded.destroy();
  }
};
const getSessionPresenceState = (
  ...
  const states = parseMollyPresenceStates(store.getAllStates());
  return Object.values(states).find(
    (state): state is MollySessionPresenceState =>
...
describe('CliPresenceRuntime local-origin plane payload', () => {
  ...
  it('excludes peers replicated from the workspace presence room', () => {
    ...
    expect(
      Object.values(parseMollyPresenceStates(getPresenceStore(presence).getAllStates())).some(
