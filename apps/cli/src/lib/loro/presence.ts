...
import {
  ...
  toMollyPresenceStreamUrl,
  type MollyMachinePresenceState,
  type MollyPresenceInstanceId,
  type MollyPresenceState,
...
export class CliPresenceRuntime {
  private readonly instanceId = uuidv4() as MollyPresenceInstanceId;
  /**
   ...
   */
  private readonly store = new EphemeralStore(MOLLY_PRESENCE_TTL_MS);
  /**
   ...
   */
  private readonly localOriginStore = new EphemeralStore(MOLLY_PRESENCE_TTL_MS);
   ...
   */
  private writeLocalOrigin(key: string, state: MollyPresenceState): void {
  ...
  attachStreams(streamsOptions: CliPresenceStreamsOptions): void {
    ...
    this.transport = new EphemeralStreamCrdt({
      streamUrl: toMollyPresenceStreamUrl(durableStreamUrl),
  ...
  private republishLocalState(): void {
    ...
    for (const key of this.sessionKeys.values()) {
      const state = this.localOriginStore.get(key) as MollySessionPresenceState | undefined;
      if (!state) continue;
  ...
  getPresenceStates(): MollyPresenceStateMap {
    return parseMollyPresenceStates(this.store.getAllStates() as Record<string, unknown>);
  }
   ...
   */
  subscribe(listener: (states: MollyPresenceStateMap) => void): () => void {
  ...
  setMachineOnline(machineId: MachineId): void {
    ...
    this.machineKey = getMollyMachinePresenceKey(machineId, this.instanceId);
    this.writeMachineHeartbeat();
  }
  ...
  writeMachineHeartbeat(): void {
    ...
    const state: MollyMachinePresenceState = {
      kind: 'machine',
  ...
  setSessionPresence(args: {
    ...
    const key = getMollySessionPresenceKey(args.sessionId, this.instanceId);
    this.sessionKeys.set(args.sessionId, key);
    const seq = ++this.sessionPresenceSeq;
    const state: MollySessionPresenceState = {
      kind: 'session',
