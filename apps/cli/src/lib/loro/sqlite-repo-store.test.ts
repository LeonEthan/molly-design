...
describe('SQLite Loro repo store', () => {
  ...
  it('creates a workspace-scoped SQLite repo store under the Molly storage directory', async () => {
    ...
    try {
      ...
      expect(cliStore.baseDir).toBe(path.join(tempHome, '.molly', 'loro-repo', 'workspace-1'));
