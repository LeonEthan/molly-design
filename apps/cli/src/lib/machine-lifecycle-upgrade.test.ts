...
describe('daemon upgrade command execution', () => {
  ...
  beforeAll(async () => {
    ...
    await writeFile(
      ...
      windows
        ? '@echo off\r\n> "%MOLLY_TEST_NPM_ARGS_FILE%" (for %%A in (%*) do @echo %%~A)\r\nexit /b %MOLLY_TEST_NPM_EXIT_CODE%\r\n'
        : '#!/bin/sh\nprintf "%s\\n" "$@" > "$MOLLY_TEST_NPM_ARGS_FILE"\nexit "$MOLLY_TEST_NPM_EXIT_CODE"\n',
      { mode: 0o755 }
    );
  ...
  it.each([0, 1])('handles npm shim exit code %i and consumes the intent', async (exitCode) => {
    ...
    vi.stubEnv('MOLLY_TEST_NPM_ARGS_FILE', argsFile);
    vi.stubEnv('MOLLY_TEST_NPM_EXIT_CODE', String(exitCode));
