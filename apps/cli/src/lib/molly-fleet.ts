...
export class MollyFleet {
  ...
  constructor(options: {
    ...
    if (this.cloudPort.kind !== 'local') {
      throw new Error('Molly Fleet supports only the local platform');
    }
  ...
  async start(): Promise<void> {
    ...
    await Promise.all([
      ...
      traceAsync(this.logger, 'startup.mcp_http', undefined, async () => {
        ...
        await startMollyMcpHttpServer({ logger: this.logger });
      }),
    ]);

    // The single implicit workspace is provisioned before bootstrap so first
    // run and restart take the same local catalog path.
  ...
  private async bootstrapFromLocalCatalog(): Promise<void> {
    ...
    // Never boot a catalog authored under a different local installation identity.
  ...
  private async dispatchLocalMachineRpc(
    message: import('@molly/shared').LocalMachineRpcRequestValidated
  ): Promise<import('@molly/shared').LocalMachineRpcResponse> {
