...
const EXCLUDE_HEADER_COMMENT =
  '# Added by Molly: runtime state and session file attachments (do not commit).';
 ...
 * `.molly/attachments/<fileId first 8 chars>-<sanitized name>`. If that name is
...
/**
 * Ensure `.molly/` is listed in the workspace's `.git/info/exclude`. Idempotent;
 ...
 *
 * This is the security boundary for the agent-facing `molly_upload_files`
