...
describe('session file blob store', () => {
  ...
  beforeEach(async () => {
    originalQuota = process.env.MOLLY_SESSION_FILE_BLOB_QUOTA_BYTES;
    originalRetainDays = process.env.MOLLY_SESSION_FILE_BLOB_RETAIN_DAYS;
    originalPlatform = process.env.MOLLY_PLATFORM;
    originalDataDir = process.env.MOLLY_DATA_DIR;
    process.env.MOLLY_PLATFORM = 'local';
    delete process.env.MOLLY_DATA_DIR;
    delete process.env.MOLLY_SESSION_FILE_BLOB_QUOTA_BYTES;
    delete process.env.MOLLY_SESSION_FILE_BLOB_RETAIN_DAYS;
  ...
  afterEach(async () => {
    if (originalQuota === undefined) {
      ...
      process.env.MOLLY_SESSION_FILE_BLOB_QUOTA_BYTES = originalQuota;
    }
    if (originalRetainDays === undefined) {
      delete process.env.MOLLY_SESSION_FILE_BLOB_RETAIN_DAYS;
    } else {
      process.env.MOLLY_SESSION_FILE_BLOB_RETAIN_DAYS = originalRetainDays;
    }
  ...
  it('rejects new writes when the local blob quota is full', async () => {
    process.env.MOLLY_SESSION_FILE_BLOB_QUOTA_BYTES = '4';
  ...
  it('preserves legacy backfilled blobs when a new write exceeds quota', async () => {
    process.env.MOLLY_SESSION_FILE_BLOB_QUOTA_BYTES = '6';
