 ...
 * the history block to `transport: 'r2'`, older builds moved the blob into
 * `_backfilled/`. Local-only Molly treats those bytes as legacy read cache.
 *
...
/**
 * Thrown when a new blob would exceed the local attachment store quota. The message is user-facing; frontends may
...
export class SessionFileBlobQuotaError extends Error {
  ...
  constructor(
    ...
    super(
      'Local attachment storage is full. Remove unneeded local sessions or attachments and try again.'
    );
...
export const getSessionFilesRoot = (homeDir: string = os.homedir()): string =>
  path.join(getMollyDataDir(undefined, homeDir), SESSION_FILES_DIR_NAME);

const getQuotaBytes = (): number => {
  const raw =
    process.env.MOLLY_SESSION_FILE_BLOB_QUOTA_BYTES ??
    process.env.LODY_SESSION_FILE_BLOB_QUOTA_BYTES;
  if (!raw) {
    return SESSION_FILE_BLOB_DEFAULT_QUOTA_BYTES;
  }
  const parsed = Number(raw);
  if (!Number.isFinite(parsed) || parsed <= 0) {
    throw new Error('Invalid MOLLY_SESSION_FILE_BLOB_QUOTA_BYTES');
  }
...
const getRetainMs = (): number => {
  const raw =
    process.env.MOLLY_SESSION_FILE_BLOB_RETAIN_DAYS ??
    process.env.LODY_SESSION_FILE_BLOB_RETAIN_DAYS;
  if (!raw) {
    return SESSION_FILE_BLOB_DEFAULT_RETAIN_MS;
  }
  const parsed = Number(raw);
  if (!Number.isFinite(parsed) || parsed < 0) {
    throw new Error('Invalid MOLLY_SESSION_FILE_BLOB_RETAIN_DAYS');
  }
 ...
 *
 * Quota: legacy backfilled blobs are no longer safe to delete because the
 * product-cloud copy is outside the local-only product boundary. Reject a new
 * write when the complete local store would exceed its quota.
 */
...
/**
 * Locate bytes retained by an older relay backfill. The history row contains
 * the relay id while `_backfilled` is keyed by the former local id, so identity
 * is recovered from the immutable size and SHA-256 stored in Session history.
 */
export const findLegacyBackfilledSessionFileBlob = async (args: {
  workspaceId: string;
  sessionId: string;
  sizeBytes: number;
  sha256: string;
  ...
  if (!isSafeId(args.workspaceId) || !isSafeId(args.sessionId)) {
    throw new Error('Invalid workspaceId or sessionId for legacy blob lookup');
  }
  const dir = path.join(
    getSessionFilesRoot(args.homeDir),
    BACKFILLED_DIR_NAME,
  ...
  for (const entry of entries) {
    if (!entry.isFile() || !isSafeId(entry.name)) continue;
    const candidate = path.join(dir, entry.name);
    const stat = await fs.promises.stat(candidate);
    if (stat.size !== args.sizeBytes) continue;
    const hash = crypto.createHash('sha256');
    for await (const chunk of fs.createReadStream(candidate)) hash.update(chunk as Buffer);
    if (hash.digest('hex') === args.sha256.toLowerCase()) return candidate;
  }
  return null;
};
