...
describe('SessionGCManager', () => {
  ...
  describe('loadGCConfig', () => {
    ...
    it('reads env vars', () => {
      const original = {
        timeout: process.env.MOLLY_SESSION_GC_IDLE_TIMEOUT_MS,
        threshold: process.env.MOLLY_SESSION_GC_MEMORY_THRESHOLD_BYTES,
      };
      process.env.MOLLY_SESSION_GC_IDLE_TIMEOUT_MS = '300000';
      process.env.MOLLY_SESSION_GC_MEMORY_THRESHOLD_BYTES = '536870912';
      try {
        const config = loadGCConfig();
        expect(config.idleTimeoutMs).toBe(300000);
        expect(config.memoryThresholdBytes).toBe(536870912);
      } finally {
        if (original.timeout === undefined) delete process.env.MOLLY_SESSION_GC_IDLE_TIMEOUT_MS;
        else process.env.MOLLY_SESSION_GC_IDLE_TIMEOUT_MS = original.timeout;
        if (original.threshold === undefined)
          delete process.env.MOLLY_SESSION_GC_MEMORY_THRESHOLD_BYTES;
        else process.env.MOLLY_SESSION_GC_MEMORY_THRESHOLD_BYTES = original.threshold;
      }
    });
  });
