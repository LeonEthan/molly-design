...
export function loadGCConfig(): SessionGCConfig {
  return {
    idleTimeoutMs: readEnvNumber('MOLLY_SESSION_GC_IDLE_TIMEOUT_MS', 20 * 60 * 1000), // 20 minutes
    sweepIntervalMs: readEnvNumber('MOLLY_SESSION_GC_SWEEP_INTERVAL_MS', 60 * 1000),
    enabled:
      (process.env.MOLLY_SESSION_GC_ENABLED ?? process.env.LODY_SESSION_GC_ENABLED) !== 'false',
    memoryThresholdBytes: readEnvNumber(
      'MOLLY_SESSION_GC_MEMORY_THRESHOLD_BYTES',
      defaultMemoryThresholdBytes()
    ),
    maxEvictionsPerCall: readEnvNumber(
      'MOLLY_SESSION_GC_MAX_EVICTIONS_PER_CALL',
      DEFAULT_MAX_EVICTIONS_PER_CALL
    ),
    pressureRecheckAttempts: readEnvNumber(
      'MOLLY_SESSION_GC_PRESSURE_RECHECK_ATTEMPTS',
      DEFAULT_PRESSURE_RECHECK_ATTEMPTS
    ),
    pressureRecheckDelayMs: readEnvNumber(
      'MOLLY_SESSION_GC_PRESSURE_RECHECK_DELAY_MS',
