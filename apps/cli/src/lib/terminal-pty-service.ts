...
import { clearManagedGhTokenEnv, MOLLY_MANAGED_GH_TOKEN_SHA256_ENV } from '@/lib/gh-token-injector';
...
const TERMINAL_ENV_BLOCKLIST = new Set([
  ...
  'MOLLY_CLI_TOKEN',
  'MOLLY_ELECTRON_BOOTSTRAP',
  'MOLLY_ELECTRON_SESSION_TOKEN',
  'MOLLY_GIT_CRED_BROKER_STATE_FILE',
  'MOLLY_GIT_CRED_BROKER_TOKEN',
  'MOLLY_GIT_CRED_BROKER_URL',
  'MOLLY_GIT_CRED_CONTEXT_TOKEN',
  MOLLY_MANAGED_GH_TOKEN_SHA256_ENV,
]);
...
function buildTerminalEnv(sessionId: string): NodeJS.ProcessEnv {
  const base: NodeJS.ProcessEnv = {
    ...
    FORCE_COLOR: '1',
    MOLLY_SESSION_ID: sessionId,
    MOLLY_WORKSPACE_SESSION_ID: sessionId,
  };
  ...
  for (const key of TERMINAL_ENV_BLOCKLIST) {
    delete merged[key];
    if (key.startsWith('MOLLY_')) delete merged[key.replace(/^MOLLY_/, 'LODY_')];
  }
  return merged;
}
