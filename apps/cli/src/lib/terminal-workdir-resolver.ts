...
function resolveDefaultChatWorkdir(
  ...
  const workdir = path.join(getMollyDataDir(undefined, homeDir), 'chats', sessionId);
  ...
  if (project?.kind === 'local') {
    ...
    if (meta.isWorktree || project.useWorktree === true) {
      ...
      return requireExistingDirectory(
        getWorktreeHostPathFromDotlodyPath(repoId, sessionId, getMollyDataDir(undefined, homeDir)),
        isDirectory
      );
    }
