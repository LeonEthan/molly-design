...
import { SessionIdSchema } from '@molly/shared';
...
/**
 * The MCP HTTP host process: `molly __internal molly-mcp-http-host`.
 *
...
export async function runMollyMcpHttpHost(): Promise<void> {
  const logger = createFileLogger('mcp-http-%DATE%.log');
  const token = process.env[MCP_HTTP_TOKEN_ENV] ?? process.env.LODY_MCP_HTTP_TOKEN;
  ...
  const preferredPortRaw =
    process.env[MCP_HTTP_PREFERRED_PORT_ENV] ?? process.env.LODY_MCP_HTTP_PREFERRED_PORT;
...
const singleHeader = (req: http.IncomingMessage, name: string): string | null => {
  const value =
    req.headers[name] ??
    (name.startsWith('x-molly-mcp-')
      ? req.headers[name.replace('x-molly-mcp-', 'x-lody-mcp-')]
      : undefined);
...
async function handleRequest(
  ...
  if (!context) {
    reject(res, 400, 'Missing or invalid Molly MCP session context headers');
    return;
  }
