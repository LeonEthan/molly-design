 ...
 * (`molly-mcp-http-server.ts`), the MCP HTTP host subprocess
 * (`molly-mcp-http-host.ts`), and the ACP client config builder
...
export interface MollyMcpHttpEndpoint {
  url: string;
  token: string;
}
...
export const MCP_HTTP_TOKEN_ENV = 'MOLLY_MCP_HTTP_TOKEN';
export const MCP_HTTP_PREFERRED_PORT_ENV = 'MOLLY_MCP_HTTP_PREFERRED_PORT';
...
export const buildMollyMcpHttpHeaders = (
  endpoint: MollyMcpHttpEndpoint,
  context: {
