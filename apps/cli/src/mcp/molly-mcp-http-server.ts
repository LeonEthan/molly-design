 ...
 * (`molly __internal molly-mcp-http-host`, see `molly-mcp-http-host.ts`).
 *
...
class McpHttpHostSupervisor {
  ...
  private endpoint: MollyMcpHttpEndpoint | null = null;
  private lastPort = 0;
  private attempts = 0;
  ...
  getEndpoint(): MollyMcpHttpEndpoint | null {
  ...
  private spawnChild(cliEntrypoint: string): ChildProcess {
    const child = spawn(process.execPath, [cliEntrypoint, '__internal', 'molly-mcp-http-host'], {
...
export const getMollyMcpHttpEndpoint = (): MollyMcpHttpEndpoint | null =>
  supervisorInstance?.getEndpoint() ?? null;

export async function startMollyMcpHttpServer(options: { logger: Logger }): Promise<void> {
  if (supervisorInstance) {
    return;
  }
  if ((process.env.MOLLY_MCP_HTTP_DISABLED ?? process.env.LODY_MCP_HTTP_DISABLED) === '1') {
    options.logger.info('[mcp-http] disabled via MOLLY_MCP_HTTP_DISABLED; using stdio MCP only');
    return;
  }
...
export async function stopMollyMcpHttpServer(): Promise<void> {
