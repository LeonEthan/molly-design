...
vi.mock('@molly/shared/node/local-ipc', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@molly/shared/node/local-ipc')>();
...
import { __mollyMcpServerInternals } from './molly-mcp-server';
