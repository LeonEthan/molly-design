...
describe('Molly Task MCP tool gate', () => {
  const taskToolNames = [
    'molly_task_list',
    'molly_task_get',
    'molly_task_create',
    'molly_task_propose',
    'molly_task_update',
    'molly_task_edit_body',
    'molly_task_comment',
  ];

  it('omits every Task tool while leaving the rest of Molly MCP available when disabled', async () => {
    const names = await listPublishedToolNames(false);
    expect(names).not.toContain('lody_feedback');
    expect(names).toEqual(
      expect.arrayContaining(['molly_session_rename', 'molly_session_rename_many'])
    );
...
describe('molly task MCP input schemas', () => {
