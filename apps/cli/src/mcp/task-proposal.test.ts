...
import { type SessionHistoryInput, type SessionId, type TaskProposalMeta } from '@molly/shared';
import { MollyOperationStoreError } from '@/orchestration/operation-store';
...
describe('publishTaskProposal', () => {
  ...
  it('reports a retryable failure until the same locally durable proposal is confirmed', async () => {
    ...
    ).rejects.toMatchObject<MollyOperationStoreError>({
