...
import type { McpServerId, WorkspaceId, WorkspaceMcpServerMeta } from '@molly/shared';
import { WorkspaceSyncUnavailableError } from '@/lib/command-runtime';
import { buildMollyMcpServer } from './molly-mcp-server';
import {
...
describe('molly_mcp_configure input', () => {
  ...
  it('is published by the Molly MCP server with both connection transports', async () => {
    ...
    try {
      ...
      const tool = result.tools.find(({ name }) => name === 'molly_mcp_configure');
