...
const StdioMcpConnectionInputSchema = z
  .object({
    ...
    envPassthrough: z
      ...
      .max(MAX_MCP_COLLECTION_ENTRIES)
      .optional()
      .describe('Environment variable names read from the target Molly daemon environment.'),
  })
  .strict();
...
export const WorkspaceMcpConfigureToolInputSchema = z
  .object({
    name: z
      ...
      .refine((name) => !namesEqual(name, 'molly') && !namesEqual(name, 'lody'), {
        message: 'The names "molly" and "lody" are reserved for the built-in Molly MCP server.',
      })
...
export async function configureWorkspaceMcpServer(
  ...
  return withWorkspaceConfigureLock(workspaceId, async () => {
    ...
    if (matches.length > 0) {
      throw new Error(
        `A workspace MCP server named ${input.name} already exists. Review and update it in Settings → MCP.`
      );
    }
