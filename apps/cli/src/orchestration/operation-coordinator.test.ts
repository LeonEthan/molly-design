...
import { MollyOperationCoordinator } from './operation-coordinator';
import { MollyOperationStore } from './operation-store';
...
const makeHarness = async (options?: {
  ...
  const storeFactory = () => {
    ...
    const factoryStore = new MollyOperationStore(storePath, options?.now ?? (() => TEST_NOW_MS));
  ...
  const coordinatorOptions = {
    ...
    materializeTarget,
  } satisfies ConstructorParameters<typeof MollyOperationCoordinator>[0];
  const coordinator = new MollyOperationCoordinator(coordinatorOptions);
  const store = new MollyOperationStore(storePath, () => TEST_NOW_MS);
  store.accept({
    workspaceId,
...
describe('LodyOperationCoordinator', () => {
  ...
  it('waits for cross-replica target catch-up before replaying a fixed turn', async () => {
    ...
    let store = new MollyOperationStore(harness.storePath, () => TEST_NOW_MS);
    expect(store.get(harness.requesterSessionId, 'review-round-1').items[0]).toMatchObject({
      status: 'active',
      inputDurable: false,
    });
    ...
    store = new MollyOperationStore(harness.storePath, () => TEST_NOW_MS);
    try {
      expect(store.get(harness.requesterSessionId, 'review-round-1').items[0]).toMatchObject({
        status: 'active',
        inputDurable: true,
  ...
  it('does not start materialization after the deadline passes during an evidence read', async () => {
    ...
    expect(harness.materializeTarget).not.toHaveBeenCalled();
    const store = new MollyOperationStore(harness.storePath, () => nowMs);
    try {
      expect(store.get(harness.requesterSessionId, 'review-round-1').items[0]).toMatchObject({
        status: 'failed',
  ...
  it('expires a Delivery 8h past its Operation deadline instead of waking the requester', async () => {
    ...
    const oldStore = new MollyOperationStore(harness.storePath, () => TEST_NOW_MS);
    try {
  ...
  it('keeps a persisted terminal assistant result at the deadline before handled catches up', async () => {
    ...
    const finalStore = new MollyOperationStore(harness.storePath, () => TEST_NOW_MS);
    try {
  ...
  it('keeps progress pending until the terminal Loro write is flushed', async () => {
    ...
    await harness.coordinator.idle();
    harness.coordinator.stop();
    const recovered = new MollyOperationStore(harness.storePath, () => TEST_NOW_MS);
    try {
      expect(
  ...
  it('retries terminal settlement after its consume write fails', async () => {
    ...
    const consume = vi.spyOn(MollyOperationStore.prototype, 'consumeClaimedDelivery');
    ...
    try {
      ...
      const afterFailure = new MollyOperationStore(harness.storePath, () => TEST_NOW_MS);
      try {
  ...
  it.each([0, 2])(
    ...
    async (historyFailuresBeforeSuccess) => {
      ...
      const originalConsume = MollyOperationStore.prototype.consumeClaimedDelivery;
      const originalRelease = MollyOperationStore.prototype.releaseDeliveryClaim;
      const consume = vi.spyOn(MollyOperationStore.prototype, 'consumeClaimedDelivery');
      const release = vi.spyOn(MollyOperationStore.prototype, 'releaseDeliveryClaim');
      consume.mockImplementation(function (this: MollyOperationStore, ...args) {
        if (writeOutage) throw new Error('terminal database write outage');
        return originalConsume.apply(this, args);
      });
      release.mockImplementation(function (this: MollyOperationStore, ...args) {
        if (writeOutage) throw new Error('terminal database write outage');
        return originalRelease.apply(this, args);
      });
      const inspect = new MollyOperationStore(harness.storePath, () => TEST_NOW_MS);
      try {
        harness.coordinator.start();
        await harness.coordinator.idle();
  ...
  it('does not write configuration failure when another Worker claims during config sync', async () => {
    ...
    harness.coordinator.start();
    await syncStarted;
    const competitorStore = new MollyOperationStore(harness.storePath, () => TEST_NOW_MS);
    try {
      expect(
  ...
  it('does not replay after graceful teardown terminalizes a started Delivery turn', async () => {
    ...
    const shutdownStore = new MollyOperationStore(harness.storePath, () => TEST_NOW_MS);
    try {
  ...
  it('keeps an active continuation pending until durable completion evidence exists', async () => {
    ...
    expect(harness.continueSession).not.toHaveBeenCalled();
    const storeWhileActive = new MollyOperationStore(harness.storePath, () => TEST_NOW_MS);
    try {
    ...
    expect(harness.continueSession).toHaveBeenCalledOnce();
    const storeAfterRetry = new MollyOperationStore(harness.storePath, () => TEST_NOW_MS);
    try {
  ...
  it('exhausts two orphaned pre-provider attempts without starting ACP a third time', async () => {
    ...
    const crashedStore = new MollyOperationStore(harness.storePath, () => TEST_NOW_MS);
    try {
  ...
  it('retries an observed settlement on a later wake without replaying provider execution', async () => {
    ...
    try {
      ...
      const strandedStore = new MollyOperationStore(harness.storePath, () => TEST_NOW_MS);
      try {
        expect(
      ...
      await harness.coordinator.idle();
      const retryStore = new MollyOperationStore(harness.storePath, () => TEST_NOW_MS);
      try {
  ...
  it('does not spend execution attempts when completion history is not durable', async () => {
    ...
    const beforeDurableHistory = new MollyOperationStore(harness.storePath, () => TEST_NOW_MS);
    try {
      expect(
  ...
  it('releases a prepared claim when the pre-provider start fence write fails', async () => {
    ...
    const markStarted = vi
      .spyOn(MollyOperationStore.prototype, 'markClaimedDeliveryExecutionStarted')
      .mockImplementationOnce(() => {
