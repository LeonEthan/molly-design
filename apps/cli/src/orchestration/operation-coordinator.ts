...
export type MollyOperationCoordinatorOptions = {
  ...
  logger: Logger;
  storeFactory?: () => MollyOperationStore;
  storePath?: string;
  now?: () => number;
...
const completionText = (operation: StoredMollyOperation): string =>
  [
    `Molly Operation ${operation.operationId} (${operation.kind}) finished.`,
...
export class MollyOperationCoordinator {
  private readonly storeFactory: () => MollyOperationStore;
  ...
  private store: MollyOperationStore | null = null;
  ...
  constructor(private readonly options: MollyOperationCoordinatorOptions) {
    const storePath = options.storePath ?? getMollyOperationStorePath(options.machineId);
    this.storeFactory = options.storeFactory ?? (() => new MollyOperationStore(storePath));
  ...
  start(): void {
    ...
    const storePath = this.options.storePath ?? getMollyOperationStorePath(this.options.machineId);
  ...
  private withStore<T>(fn: (store: MollyOperationStore) => T): T {
  ...
  private async reconcileOperation(operation: StoredMollyOperation): Promise<void> {
    ...
    let items: MollyOperationItemResult[];
    try {
    ...
    if (items.every((item) => item.status !== 'active')) {
      const completion: MollyOperationCompletion = { type: 'result', value: { items } };
      latestOperation = this.withStore((store) =>
  ...
  private async writeOperationProgress(operation: StoredMollyOperation): Promise<void> {
  ...
  private async reconcileItem(
    operation: StoredMollyOperation,
    item: MollyOperationItemResult,
    index: number,
    isAtDeadline: () => boolean,
    signal: AbortSignal
  ): Promise<MollyOperationItemResult> {
    ...
    if (userTurn?.status === 'failed') {
      return {
        ...
        ...(item.label ? { label: item.label } : {}),
        target: item.target,
        error: makeMollyError('TARGET_FAILED', 'Target Turn failed.', false),
      };
    }
    if (userTurn?.status === 'canceled') {
      return {
  ...
  private armDeadline(operation: StoredMollyOperation): void {
    const key = this.operationKey(operation);
  ...
  private hasMaterializationRetry(operation: StoredMollyOperation, itemIndex: number): boolean {
    return this.materializationTimers.has(this.materializationKey(operation, itemIndex));
  }

  private clearMaterializationRetry(operation: StoredMollyOperation, itemIndex: number): void {
    const key = this.materializationKey(operation, itemIndex);
  ...
  private clearOperationMaterializationRetries(operation: StoredMollyOperation): void {
  ...
  private clearDeadline(operation: StoredMollyOperation): void {
    const key = this.operationKey(operation);
  ...
  private operationKey(operation: StoredMollyOperation): string {
    return `${operation.requesterSessionId}\0${operation.operationId}`;
  }

  private materializationKey(operation: StoredMollyOperation, itemIndex: number): string {
  ...
  private enqueueDelivery(delivery: StoredMollyDelivery, reason: string): void {
  ...
  private isDeliveryPending(delivery: StoredMollyDelivery): boolean {
  ...
  private async deliverIfRunnable(delivery: StoredMollyDelivery, reason: string): Promise<void> {
    if (!this.started) return;
  ...
  private async writeCompletionTurn(
    ...
    const buildTurn = (progressMessageId: string | undefined): SessionHistoryInput => {
