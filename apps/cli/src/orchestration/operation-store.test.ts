...
const makeStore = async (now: () => number = () => Date.parse('2026-07-20T00:00:00Z')) => {
  ...
  roots.add(root);
  return new MollyOperationStore(path.join(root, 'operations.sqlite3'), now);
};
...
describe('LodyOperationStore', () => {
  it('restricts the store directory and database to the local account', async () => {
    ...
    const store = new MollyOperationStore(dbPath);
    try {
  ...
  it('marks an existing legacy pending Delivery as execution-uncertain', async () => {
    ...
    const migrated = new MollyOperationStore(dbPath);
    try {
  ...
  it('exposes a Delivery inserted by a legacy writer after the store opens', async () => {
    ...
    roots.add(root);
    const dbPath = path.join(root, 'operations.sqlite3');
    const preTriggerStore = new MollyOperationStore(dbPath);
    ...
    const store = new MollyOperationStore(dbPath, undefined, { maintenance: false });
    try {
      legacyWriter.exec(`
        BEGIN IMMEDIATE;
        UPDATE operations
  ...
  it('rejects operation id reuse with different semantic input', async () => {
    ...
    try {
      ...
      expect(() =>
        ...
        expect.objectContaining<MollyOperationStoreError>({ code: 'OPERATION_ID_REUSED' })
      );
    } finally {
      store.close();
    }
  ...
  it('scopes lookup to the requester Session', async () => {
    ...
    try {
      ...
      expect(() => store.get('another-session' as SessionId, 'review-round-1')).toThrowError(
        expect.objectContaining<MollyOperationStoreError>({ code: 'OPERATION_NOT_FOUND' })
      );
    } finally {
      store.close();
    }
  ...
  it('shares accepted intent across local processes and adopts it after reopen', async () => {
    ...
    roots.add(root);
    const dbPath = path.join(root, 'operations.sqlite3');
    const acceptingProcess = new MollyOperationStore(dbPath);
    const leaseOwnerProcess = new MollyOperationStore(dbPath);
    try {
    ...
    const replacementLeaseOwner = new MollyOperationStore(dbPath);
    try {
...
describe('canonicalizeLodyCommand', () => {
  it('sorts object keys but preserves array order', () => {
    expect(canonicalizeMollyCommand({ b: 2, a: [{ z: 1, y: 2 }, 3] })).toBe(
      '{"a":[{"y":2,"z":1},3],"b":2}'
    );
  });
});
...
describe('maintenance-free open', () => {
  it('migrates a current delivery schema that lacks progress settlements', async () => {
    ...
    roots.add(root);
    const dbPath = path.join(root, 'operations.sqlite3');
    const initial = new MollyOperationStore(dbPath);
    initial.close();
    ...
    const migrated = new MollyOperationStore(dbPath, undefined, { maintenance: false });
    try {
      expect(
  ...
  it('does not acquire the SQLite writer lock when the schema is current', async () => {
    ...
    roots.add(root);
    const dbPath = path.join(root, 'operations.sqlite3');
    const owner = new MollyOperationStore(dbPath);
    owner.close();

    const writer = new Database(dbPath);
    writer.exec('BEGIN IMMEDIATE');
    try {
      const nonOwner = new MollyOperationStore(dbPath, undefined, { maintenance: false });
      try {
        expect(nonOwner.listPendingDeliveries('workspace-1' as WorkspaceId)).toEqual([]);
      } finally {
        nonOwner.close();
      }
    } finally {
  ...
  it('skips open-time repair/cleanup writes but still serves reads and writes', async () => {
    ...
    const nonOwner = new MollyOperationStore(dbPath, now, { maintenance: false });
    try {
    ...
    const owner = new MollyOperationStore(dbPath, now);
    try {
...
describe('runWithOperationStoreBusyRetry', () => {
  ...
  it('rethrows non-busy errors immediately without sleeping', async () => {
    ...
    await expect(
      runWithOperationStoreBusyRetry(
        () => {
          throw new MollyOperationStoreError('OPERATION_NOT_FOUND', 'missing', false);
        },
        {
          sleep: async (ms) => {
            sleeps.push(ms);
...
it('retains unfinished progress ownership across delivery consumption, reopening and retention cleanup', async () => {
  ...
  let store = new MollyOperationStore(dbPath, () => now);
  store.accept(input);
  ...
  store = new MollyOperationStore(dbPath, () => now);
  try {
