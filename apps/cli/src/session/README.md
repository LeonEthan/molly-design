...
recovered without duplicate input. The control-plane Mirror never materializes history.
...
`hasPendingUserTurnActivation` in `@molly/shared` is the single answer, and why
...
assistant entry" logic (`@molly/shared` `schema.ts` terminal predicate) treats it as done.
...
The generic five-second process escalation does not apply to native Grok closure,
including Stop during restoration or configuration. A ten-second close wait reports a
diagnostic without discarding its raw request; retries share that request until it settles.
Late `closed`/`notResident` evidence still belongs to the original session and connection.
A rejected close can be retried explicitly on that connection, including the legacy path
without an Effect runtime. Retry Stop leaves dispatch paused; Continue clears the pause
only after native requests and original artifact processing finish. Neither recovery path
terminates the process before requesting native closure.
