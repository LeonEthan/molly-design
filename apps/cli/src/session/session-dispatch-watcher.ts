...
export class SessionDispatchWatcher {
  ...
  private async promoteNextQueuedMessage(
    ...
    try {
      if (this.deps.executionService.isDispatchPausedInMemory(meta.id)) return null;
      const freshMeta = await sessionDoc.getMetaState();
      if (freshMeta?.dispatchPause?.state === 'paused') return null;
      const queuedItem = await sessionDoc.peekReadyMessageQueue();
      if (!queuedItem) {
        return null;
      }

      const queuedTurnId = queuedItem.userTurnId?.trim() || `queued-${queuedItem.$cid}`;
      const existingTurn = history.find((entry) => entry.id === queuedTurnId);
      if (existingTurn) {
        const currentMeta = freshMeta ?? meta;
        // History can commit before its activation pointer. Retain the queue
        // until the exact pending turn is activated; never rewind a newer input.
        if (
          existingTurn.role === 'user' &&
          (existingTurn.status === 'pending' || existingTurn.status === 'seen') &&
          currentMeta.lastHandledUserMsgId !== queuedTurnId
        ) {
          const activeInput = getPendingUserTurnActivationId(currentMeta);
          if (activeInput && activeInput !== queuedTurnId) return null;
      ...
      if (this.deps.executionService.isDispatchPausedInMemory(meta.id)) return null;
      const beforeWrite = await sessionDoc.getMetaState();
      if (
        beforeWrite?.dispatchPause?.state === 'paused' ||
        this.deps.executionService.isDispatchPausedInMemory(meta.id)
      )
        return null;
      // Persist history before removing the queue item. A crash between them is
      // recovered by the stable queued turn id above; invalid writes retain input.
