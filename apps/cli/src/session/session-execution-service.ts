...
class DispatchPaused extends Error {}
...
type TurnRuntimeState = {
  ...
  retryCancellation?: () => Promise<boolean>;
...
export class SessionExecutionService {
  ...
  private readonly pendingStopRecovery = new Map<
    SessionId,
    { turnId: string; close: () => Promise<void>; canvasTurnId?: string }
  >();
  ...
  private async steerSessionLocked(options: {
    ...
    try {
      ...
      const steerRun = agentClient.steerPrompt(acpSessionId, promptBlocks, {
        onLateApplied: async () => {
          const settled = await sessionDoc.sessionData.commands.applyHistoryAction({
            kind: 'user-status',
            turnId: options.userTurnId,
            status: runtime.cancelRequested ? 'canceled' : 'handled',
            steerProjection: true,
            authoritativeDelivery: true,
            deliveredSteer: true,
          });
          if (settled.matched)
            await this.acknowledgeSteerTurn(options.sessionId, options.userTurnId);
        },
      });
  ...
  async reconcileSteerHistory(
    ...
    await this.steerStatusQueue.enqueue(sessionId, async () => {
      ...
      for (const [turnId, status] of Object.entries(statuses)) {
        ...
        const terminal =
          !!turn.turn.status &&
          !['pending_apply', 'pending', 'seen', 'processing', 'delivery_unknown'].includes(
            turn.turn.status
          );
  ...
  private requestAgentCancelInBackground(runtime: TurnRuntimeState, stage: string): void {
    ...
    if (!runtime.stopClosure) {
      ...
      const wait = client.closeAfterStop?.(acpSessionId);
      runtime.stopClosure = client.nativeStopCompletion ?? wait;
      if (wait) {
        const providerSettlement = runtime.providerPromptSettlement;
        const pendingRequests = [
          client.pendingPromptCompletion,
          ...(runtime.pendingSteerConfig ?? []),
        ];
        runtime.retryStopClosure = async () => {
          const retry = client.retryCloseAfterStop(acpSessionId);
          runtime.stopClosure = client.nativeStopCompletion ?? retry;
          void runtime.stopClosure.catch(() => {});
          this.observeStopClosureWait(runtime, retry);
          await retry;
          await Promise.allSettled(pendingRequests);
          await providerSettlement;
        };
        this.observeStopClosureWait(runtime, wait);
      }
  ...
  private drainCancelledPrompt(session: ISession, runtime?: TurnRuntimeState): Promise<void> {
    ...
    if (session.agentClient?.requiresNativeStopClosure || runtime?.stopClosure) {
      // Native session.close must keep its transport for retry and late evidence.
      // A timeout never authorizes killing this process or releasing its owner.
      if (runtime) {
        runtime.cancellationDrain = pendingPrompt;
        runtime.retryCancellation = async () => {
          try {
            await runtime.retryStopClosure?.();
            return true;
          } catch {
            return false; // The bounded close waiter records the diagnostic.
          }
        };
      }
    ...
    const owner = createCancellationDrain({
      pending: pendingPrompt,
      timeoutMs: 5_000,
      terminate: () => session.terminate(true),
      onFailure: (error) => {
        this.deps.logger.error(
    ...
    if (runtime) {
      runtime.cancellationDrain = drain;
      runtime.retryCancellation = owner.retry;
    }
    return drain;
  }
  ...
  private finalizeCancelledTurnEffect(
    ...
    return Effect.gen(function* () {
      ...
      const nativeClose = sessionToTerminate?.agentClient?.requiresNativeStopClosure;
      if (nativeClose && runtime) self.requestAgentCancelInBackground(runtime, 'preparing');
      if (options.terminateSession && sessionToTerminate && !nativeClose) {
      ...
      if (sessionToDrain && (!options.terminateSession || nativeClose)) {
  ...
  private async runVisibleSessionTurn(
      ...
      Effect.acquireRelease(Effect.succeed(runtime), (turnRuntime, exit) =>
        ...
        Effect.flatMap(() =>
          Effect.gen(function* () {
            ...
            const dispatchMeta = yield* self.tryPromise(() => self.getSessionMeta(sessionId));
            if (
              !runtime.cancelRequested &&
              (dispatchMeta?.dispatchPause?.state === 'paused' ||
                self.isDispatchPausedInMemory(sessionId))
            ) {
              yield* Effect.fail(new DispatchPaused('Explicit continuation is required'));
            }
    ...
    try {
      ...
      if (error instanceof DispatchPaused) {
        outcome = 'dispatch-paused';
      } else if (isSessionTurnClaimContended(error)) {
      ...
      try {
        if (runtime.cancelRequested && runtime.session)
          await this.drainCancelledPrompt(runtime.session, runtime);
        ...
        if (runtime.retryStopClosure)
          this.pendingStopRecovery.set(sessionId, {
            ...
            close: async () => {
              await runtime.retryStopClosure?.();
              await runtime.providerPromptSettlement;
              await runtime.canvasFinalization;
            },
  ...
  private async upsertSessionMeta(
    ...
    if ('dispatchPause' in patch) await this.deps.workspaceDocument.repo.flush?.();
  }
  ...
  async cancelSession(
    ...
    const pendingInput =
      options.pauseDispatch === false ? (options.pendingInput ?? 'preserve') : 'promote';
    ...
    if (message.action === 'resume') {
      const meta = await this.getSessionMeta(sessionId);
      if (meta?.dispatchPause?.turnId !== turnId)
        return { success: false, error: 'The stopped turn changed. Reload and retry.' };
      if (this.turnRuntimeBySession.has(sessionId)) {
        await this.cancelSession({ ...message, action: undefined }, options);
        return {
          success: false,
          error: 'Stopping the previous execution. Continue after it has ended.',
        };
      }
      const recovery = this.pendingStopRecovery.get(sessionId);
      if (recovery) {
        if (recovery.turnId !== turnId)
          return { success: false, error: 'The execution owner changed. Reload and retry.' };
        try {
          await recovery.close();
          if (recovery.canvasTurnId)
            this.deps.releaseDesignCanvas?.(sessionId, recovery.canvasTurnId);
          this.pendingStopRecovery.delete(sessionId);
        } catch (error) {
          return { success: false, error: formatErrorMessage(error) };
        }
      }
      const latest = await this.getSessionMeta(sessionId);
      if (latest?.dispatchPause?.turnId !== turnId || this.turnRuntimeBySession.has(sessionId))
        return { success: false, error: 'The execution owner changed. Reload and retry.' };
      await this.upsertSessionMeta(sessionId, {
        dispatchPause: { turnId, state: 'resumed' },
        lastCanceledTurn: undefined,
      });
    ...
    if (retainedRecovery?.turnId === turnId && !this.turnRuntimeBySession.has(sessionId)) {
      try {
        await retainedRecovery.close();
        if (retainedRecovery.canvasTurnId)
          this.deps.releaseDesignCanvas?.(sessionId, retainedRecovery.canvasTurnId);
        if (this.pendingStopRecovery.get(sessionId) === retainedRecovery)
          this.pendingStopRecovery.delete(sessionId);
        return { success: true };
      } catch (error) {
        await this.upsertSessionMeta(sessionId, {
          dispatchPause: { turnId, state: 'paused', error: formatErrorMessage(error) },
        });
        return { success: false, error: formatErrorMessage(error) };
      }
    }
            ...
            if (liveTurnId == null) {
              ...
              if (hasUnfinishedRequestedTurn) {
                if (options.pauseDispatch !== false) {
                  const meta = await this.getSessionMeta(sessionId);
                  if (meta?.dispatchPause?.turnId !== turnId) {
                    this.stopRequestedBySession.set(sessionId, turnId);
                    await this.upsertSessionMeta(sessionId, {
                      dispatchPause: { turnId, state: 'paused' },
                    });
                  }
                }
                this.deps.logger.debug(
    ...
    if (stoppingRuntime && !stoppingRuntime.cancelRequested) {
      ...
      stoppingRuntime.terminateSessionOnCancel =
        !stoppingRuntime.promptStarted && options.prePromptSession !== 'keep';
      stoppingRuntime.cancelRequested = true;
    ...
    if (runtime) {
      ...
      if (
        ...
        (runtime.stopClosure ||
          runtimeSession?.agentClient?.pendingPromptCompletion ||
          runtime.pendingSteerConfig?.size)
      ) {
        // Retry the retained owner; neither this request nor a timeout releases it.
        void runtime.retryCancellation?.();
        return { success: true };
      }
    ...
    // This legacy ownership path may have a native request without a local
    // Effect runtime. Keep its original identity and a reachable recovery action.
    this.markCurrentTurn(sessionId, turnId);
    ...
    const recovery = {
      ...
      canvasTurnId: turnId,
      close: async () => {
        if (stoppedClient.requiresNativeStopClosure) {
          await stoppedClient.retryCloseAfterStop(stoppedAcpSessionId);
          await stoppedClient.pendingPromptCompletion;
        } else {
          await session.terminate(true);
        }
        await finalize();
    ...
    this.pendingStopRecovery.set(sessionId, recovery);
    void stoppedClient.cancel(stoppedAcpSessionId).catch((error: unknown) => {
      this.deps.logger.warn(`[${sessionId}] Cancel request failed: ${formatErrorMessage(error)}`);
    });
    try {
      await Promise.all([
        stoppedClient.closeAfterStop?.(stoppedAcpSessionId),
        this.drainCancelledPrompt(session),
      ]);
      await finalize();
      if (this.pendingStopRecovery.get(sessionId) === recovery)
        this.pendingStopRecovery.delete(sessionId);
      this.deps.releaseDesignCanvas?.(sessionId, turnId);
      return { success: true };
    } catch (error) {
      const errorMessage = formatErrorMessage(error);
      this.deps.logger.error(`[${sessionId}] Failed to stop session: ${errorMessage}`);
      await this.upsertSessionMeta(sessionId, {
        dispatchPause: { turnId, state: 'paused', error: errorMessage },
      });
