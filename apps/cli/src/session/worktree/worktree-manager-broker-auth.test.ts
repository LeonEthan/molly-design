...
import type { RepoId } from '@molly/shared';
...
describe('WorktreeManager host git credential broker routing', () => {
  ...
  afterEach(() => {
    ...
    delete process.env.MOLLY_GIT_CRED_BROKER_URL;
    delete process.env.MOLLY_GIT_CRED_BROKER_TOKEN;
  ...
  it('fetches with the caller-supplied broker, not the process-global pointer', async () => {
    ...
    process.env.MOLLY_GIT_CRED_BROKER_URL = 'http://127.0.0.1:44102';
    process.env.MOLLY_GIT_CRED_BROKER_TOKEN = 'other-workspace-token';
    ...
    expect(env.MOLLY_GIT_CRED_BROKER_URL).toBe('http://127.0.0.1:33215');
    expect(env.MOLLY_GIT_CRED_BROKER_TOKEN).toBe('session-workspace-token');
  });
  ...
  it('leaves the ambient pointer in place when no broker auth is supplied', async () => {
    ...
    process.env.MOLLY_GIT_CRED_BROKER_URL = 'http://127.0.0.1:44102';
    process.env.MOLLY_GIT_CRED_BROKER_TOKEN = 'ambient-token';

    const manager = await newManager();
    await manager.ensureRepo();

    const env = envOfGitCall('fetch');
    expect(env.MOLLY_GIT_CRED_BROKER_URL).toBe('http://127.0.0.1:44102');
    expect(env.MOLLY_GIT_CRED_BROKER_TOKEN).toBe('ambient-token');
  });
});
