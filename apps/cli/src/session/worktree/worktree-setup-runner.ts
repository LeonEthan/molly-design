...
function buildStepInput(shell: WorktreeSetupShell, command: string, step: RunningStep): string {
  if (shell === 'powershell') {
    return [
      command,
      '$__molly_status = if ($?) { 0 } elseif ($LASTEXITCODE -is [int]) { $LASTEXITCODE } else { 1 }',
      `[Console]::Out.WriteLine("${step.markerPrefix}{0}${step.markerSuffix}", $__molly_status)`,
      `[Console]::Error.WriteLine("${step.markerPrefix}{0}${step.markerSuffix}", $__molly_status)`,
      '',
    ].join('\n');
  }
...
async function runWorktreeScript(options: {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const child = spawn(command, args, {
      ...
      env: {
        ...process.env,
        MOLLY_WORKSPACE_ID: options.workspaceId,
        MOLLY_SESSION_ID: options.sessionId,
        MOLLY_WORKTREE_PATH: options.workdir,
        MOLLY_WORKTREE_BRANCH: options.branch,
        MOLLY_WORKTREE_SCRIPT_PHASE: options.phase,
        ...(options.repoFullName ? { MOLLY_GITHUB_REPO: options.repoFullName } : {}),
        ...(options.localProjectId ? { MOLLY_LOCAL_PROJECT_ID: options.localProjectId } : {}),
      },
    ...
    const runStep = async (
      ...
      const markerPrefix = `__MOLLY_WORKTREE_STEP_DONE_${markerToken}_${stepIndex}:`;
