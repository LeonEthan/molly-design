...
describe('resolveDnsResultOrder', () => {
  ...
  it('honors MOLLY_DNS_RESULT_ORDER overrides', () => {
    expect(resolveDnsResultOrder(envOf({ MOLLY_DNS_RESULT_ORDER: 'ipv6first' }), [])).toEqual({
      order: 'ipv6first',
      reason: 'env',
    });
  });
  ...
  it('treats runtime-default values as a no-op', () => {
    for (const value of ['node', 'native', 'default', 'verbatim', 'VERBATIM']) {
      expect(resolveDnsResultOrder(envOf({ MOLLY_DNS_RESULT_ORDER: value }), [])).toEqual({
        order: null,
  ...
  it('falls back to ipv4first on invalid env values', () => {
    expect(resolveDnsResultOrder(envOf({ MOLLY_DNS_RESULT_ORDER: 'bogus' }), [])).toEqual({
      order: 'ipv4first',
      reason: 'env-invalid',
    });
  });
