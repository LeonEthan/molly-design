...
export function resolveDnsResultOrder(
  ...
  const configured = (env.MOLLY_DNS_RESULT_ORDER ?? env.LODY_DNS_RESULT_ORDER)
    ?.trim()
    .toLowerCase();
  if (configured) {
...
export function applyDefaultDnsResultOrder(
  ...
  if (resolution.reason === 'env-invalid') {
    logger.warn(
      `[dns] invalid MOLLY_DNS_RESULT_ORDER value; expected ipv4first|ipv6first|verbatim|node — falling back to ipv4first`
    );
  }
