...
let originalMollyDataDir: string | undefined;
...
beforeEach(() => {
  ...
  originalMollyDataDir = process.env.MOLLY_DATA_DIR;
  process.env.MOLLY_DATA_DIR = path.join(testHomeDir, '.lody');
  vi.resetModules();
});

afterEach(() => {
  if (originalMollyDataDir === undefined) {
    delete process.env.MOLLY_DATA_DIR;
  } else {
    process.env.MOLLY_DATA_DIR = originalMollyDataDir;
  }
  try {
