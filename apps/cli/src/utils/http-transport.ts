...
export function resolveCliHttpTransportConfig(
  ...
  const transportMode = normalizeEnvValue(env.MOLLY_HTTP_TRANSPORT ?? env.LODY_HTTP_TRANSPORT);
  const transportDisabled =
    transportMode !== undefined &&
    (NODE_DEFAULT_TRANSPORT_VALUES.has(transportMode) ||
      DISABLED_TRANSPORT_VALUES.has(transportMode));
  const allowH2 = isEnabledEnvValue(env.MOLLY_HTTP2 ?? env.LODY_HTTP2);
  const connectTimeoutMs = parsePositiveIntegerEnv(
    env.MOLLY_HTTP_CONNECT_TIMEOUT_MS ?? env.LODY_HTTP_CONNECT_TIMEOUT_MS,
    DEFAULT_CONNECT_TIMEOUT_MS
  );
  const maxConcurrentStreams = parsePositiveIntegerEnv(
    env.MOLLY_HTTP2_MAX_CONCURRENT_STREAMS ?? env.LODY_HTTP2_MAX_CONCURRENT_STREAMS,
    DEFAULT_MAX_CONCURRENT_STREAMS
  );
  const diagnosticsEnabled = isEnabledEnvValue(
    env.MOLLY_HTTP_TRANSPORT_DIAGNOSTICS ?? env.LODY_HTTP_TRANSPORT_DIAGNOSTICS
  );
