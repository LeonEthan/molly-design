...
import { cleanupExpiredLogs, MOLLY_LOG_RETENTION_MS } from './log-retention';
...
describe('cleanupExpiredLogs', () => {
  it('removes managed log files older than the retention window', () => {
    ...
    touchWithMtime(oldLog, now - MOLLY_LOG_RETENTION_MS - 1);
    touchWithMtime(oldJsonl, now - MOLLY_LOG_RETENTION_MS - 1);
    touchWithMtime(recentLog, now - 60_000);
    touchWithMtime(oldText, now - MOLLY_LOG_RETENTION_MS - 1);
