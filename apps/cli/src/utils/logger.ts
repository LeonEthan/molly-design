...
import { cleanupExpiredLogs, MOLLY_LOG_DIR, MOLLY_LOG_RETENTION_MAX_FILES } from './log-retention';
...
const createFileTransport = (config: LoggerConfig) => {
  ...
  const dirname = fileConfig.dirname || MOLLY_LOG_DIR;
  ...
  return new DailyRotateFile({
    ...
    maxFiles: fileConfig.maxFiles || MOLLY_LOG_RETENTION_MAX_FILES,
...
export const createFileLogger = (filename?: string, dirname?: string): Logger => {
  const logger = createLogger({
    ...
    file: {
      ...
      dirname: dirname || MOLLY_LOG_DIR,
      maxSize: '20m',
      maxFiles: MOLLY_LOG_RETENTION_MAX_FILES,
      zippedArchive: true,
    },
  });
  rootLogger = logger;
