...
import { REGISTRY_ACP_AGENTS } from '@molly/shared';
...
describe('resolveBuiltinACPSetting', () => {
  ...
  it('ignores legacy local Codex ACP env overrides in the sync resolver', () => {
    const previousPath = process.env.MOLLY_LOCAL_CODEX_ACP_PATH;
    const previousEnabled = process.env.MOLLY_LOCAL_CODEX_ACP;
    process.env.MOLLY_LOCAL_CODEX_ACP_PATH = '/tmp/local-acp-extension-codex';
    process.env.MOLLY_LOCAL_CODEX_ACP = '1';

    try {
      expect(() => resolveBuiltinACPSetting('codex')).toThrow(/resolveACPProcessLaunchAsync/);
    } finally {
      if (previousPath === undefined) {
        delete process.env.MOLLY_LOCAL_CODEX_ACP_PATH;
      } else {
        process.env.MOLLY_LOCAL_CODEX_ACP_PATH = previousPath;
      }
      if (previousEnabled === undefined) {
        delete process.env.MOLLY_LOCAL_CODEX_ACP;
      } else {
        process.env.MOLLY_LOCAL_CODEX_ACP = previousEnabled;
      }
    }
  });
