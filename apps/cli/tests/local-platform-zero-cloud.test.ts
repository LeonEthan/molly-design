...
describe('local platform zero-cloud integration', () => {
  ...
  beforeEach(async () => {
    ...
    process.env.MOLLY_AUTH_URL = trapUrl;
    process.env.MOLLY_AUTH_SITE_URL = trapUrl;
    process.env.MOLLY_SERVER_URL = trapUrl;
  });
  ...
  it('bootstraps identity, workspace, and the local data plane without touching cloud endpoints', async () => {
    ...
    expect(process.env.MOLLY_AUTH_URL).toBeUndefined();
    expect(process.env.MOLLY_AUTH_SITE_URL).toBeUndefined();
    expect(process.env.MOLLY_SERVER_URL).toBeUndefined();
