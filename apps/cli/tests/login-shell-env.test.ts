...
describe('login-shell-env opt-out', () => {
  afterEach(() => {
    delete process.env.MOLLY_DISABLE_SHELL_ENV;
    resetLoginShellEnvCache();
  });

  it('MOLLY_DISABLE_SHELL_ENV=1 yields an empty overlay without spawning a shell', async () => {
    process.env.MOLLY_DISABLE_SHELL_ENV = '1';
