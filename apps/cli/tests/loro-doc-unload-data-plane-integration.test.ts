...
describe('session GC unloads the repo doc and invalidates its local data-plane room', () => {
  ...
  beforeEach(async () => {
    ...
    process.env.MOLLY_DATA_DIR = path.join(tempDir, '.lody-oss');
  });
  ...
  it('reopens SQLite after an interrupted queue activation and never duplicates the persisted input', async () => {
    const first = await createHarness();
    const sessionId = first.sessionId;
    const promote = async (manager: LoroDocumentManager) => {
      ...
      const watcher = new SessionDispatchWatcher({
        ...
        executionService: {
          ...
          isDispatchPausedInMemory: () => false,
        },
      } as never);
      return watcher['promoteNextQueuedMessage'](doc, (await doc.getMetaState())!, [
        ...doc.sessionData.history.readAll(),
      ]);
    };
    try {
      const doc = await first.manager.getOrCreateSessionDoc(sessionId);
      await doc.pushMessageQueue({
        task: 'synthetic retained input',
        userTurnId: 'interrupted-input',
        timestamp: '2026-09-17T00:00:00Z',
        userId: 'local-user',
        acpSessionConfig: {
          prompt: 'synthetic retained input',
          cliType: 'builtin',
          agentType: 'claude',
        },
      } as never);
      await first.manager.repo.flush();
      const update = first.manager.repo.upsertDocMeta.bind(first.manager.repo);
      const failure = vi
        .spyOn(first.manager.repo, 'upsertDocMeta')
        .mockImplementation(async (id, patch, ...rest) => {
          if (id === getSessionRoomId(sessionId) && patch.latestUserMsgId === 'interrupted-input')
            throw new Error('interrupted activation');
          return update(id, patch, ...rest);
        });
      await expect(promote(first.manager)).rejects.toThrow('interrupted activation');
      failure.mockRestore();
      // Explicit durable boundary; the interrupted operation never returned success.
      await first.manager.repo.flush();
    } finally {
      await first.dispose();
    }
    ...
    try {
      const doc = await reopened.manager.getOrCreateSessionDoc(sessionId);
      expect(doc.sessionData.history.readAll().map((entry) => entry.id)).toEqual([
        first.cliEntryId,
        'interrupted-input',
      ]);
      expect(await doc.getMessageQueue()).toHaveLength(1);
      expect((await promote(reopened.manager))?.id).toBe('interrupted-input');
      expect(await promote(reopened.manager)).toBeNull();
      expect(await doc.getMessageQueue()).toEqual([]);
      await reopened.manager.repo.flush();
    } finally {
      await reopened.dispose();
    }
    const finalOpen = await createHarness();
    try {
      const doc = await finalOpen.manager.getOrCreateSessionDoc(sessionId);
      expect(doc.sessionData.history.readAll().map((entry) => entry.id)).toEqual([
        first.cliEntryId,
        'interrupted-input',
      ]);
      expect((await doc.getMetaState())?.latestUserMsgId).toBe('interrupted-input');
      expect(await doc.getMessageQueue()).toEqual([]);
    } finally {
      await finalOpen.dispose();
    }
  });
});
