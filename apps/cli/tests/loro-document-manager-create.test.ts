...
import { LocalLoroDataPlaneServer } from '@molly/shared/local-loro-data-plane-server';
...
vi.mock('@molly/shared', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@molly/shared')>();
  return {
    ...actual,
...
vi.mock('../src/utils/const', async (importOriginal) => {
  ...
  return {
    ...actual,
    MOLLY_AUTH_URL: 'https://convex.example.test',
    MOLLY_AUTH_SITE_URL: undefined,
  };
});
...
const originalSyncTimeoutEnv = process.env.MOLLY_LORO_SYNC_META_TIMEOUT_MS;
const originalReconnectDelayEnv = process.env.MOLLY_LORO_AUTO_RECONNECT_DELAY_MS;
const originalReconnectMaxDelayEnv = process.env.MOLLY_LORO_AUTO_RECONNECT_MAX_DELAY_MS;
const originalReconnectIntervalEnv = process.env.MOLLY_LORO_AUTO_RECONNECT_INTERVAL_MS;
...
describe('LoroDocumentManager.create degraded startup behavior', () => {
  beforeEach(async () => {
    ...
    delete process.env.MOLLY_LORO_SYNC_META_TIMEOUT_MS;
    delete process.env.MOLLY_LORO_AUTO_RECONNECT_DELAY_MS;
    delete process.env.MOLLY_LORO_AUTO_RECONNECT_MAX_DELAY_MS;
    delete process.env.MOLLY_LORO_AUTO_RECONNECT_INTERVAL_MS;
  });
  ...
  afterAll(() => {
    if (originalSyncTimeoutEnv === undefined) {
      ...
      process.env.MOLLY_LORO_SYNC_META_TIMEOUT_MS = originalSyncTimeoutEnv;
    }

    if (originalReconnectDelayEnv === undefined) {
      delete process.env.MOLLY_LORO_AUTO_RECONNECT_DELAY_MS;
    } else {
      process.env.MOLLY_LORO_AUTO_RECONNECT_DELAY_MS = originalReconnectDelayEnv;
    }

    if (originalReconnectMaxDelayEnv === undefined) {
      delete process.env.MOLLY_LORO_AUTO_RECONNECT_MAX_DELAY_MS;
    } else {
      process.env.MOLLY_LORO_AUTO_RECONNECT_MAX_DELAY_MS = originalReconnectMaxDelayEnv;
    }

    if (originalReconnectIntervalEnv === undefined) {
      delete process.env.MOLLY_LORO_AUTO_RECONNECT_INTERVAL_MS;
    } else {
      process.env.MOLLY_LORO_AUTO_RECONNECT_INTERVAL_MS = originalReconnectIntervalEnv;
    }
  });
  ...
  it('continues in degraded mode when meta room sync times out', async () => {
    process.env.MOLLY_LORO_SYNC_META_TIMEOUT_MS = '1';
  ...
  it('sweeps rooms via repo.reconnect on the healthy watchdog tick', async () => {
    ...
    vi.useFakeTimers();
    process.env.MOLLY_LORO_AUTO_RECONNECT_INTERVAL_MS = '10';
  ...
  it('settles a queued manual reconnect when cleanup runs first', async () => {
    process.env.MOLLY_LORO_AUTO_RECONNECT_INTERVAL_MS = '0';
  ...
  it('reconnects and notifies listeners after meta room disconnects', async () => {
    vi.useFakeTimers();
    process.env.MOLLY_LORO_AUTO_RECONNECT_DELAY_MS = '1';
    process.env.MOLLY_LORO_AUTO_RECONNECT_INTERVAL_MS = '0';
  ...
  it('backs off automatic reconnects while the meta room remains disconnected', async () => {
    vi.useFakeTimers();
    process.env.MOLLY_LORO_AUTO_RECONNECT_DELAY_MS = '100';
    process.env.MOLLY_LORO_AUTO_RECONNECT_MAX_DELAY_MS = '1000';
    process.env.MOLLY_LORO_AUTO_RECONNECT_INTERVAL_MS = '0';
