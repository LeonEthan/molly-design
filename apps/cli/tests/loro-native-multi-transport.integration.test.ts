...
const originalWaitMetaTimeoutEnv = process.env.MOLLY_LORO_WAIT_META_SYNC_TIMEOUT_MS;
...
describe('native multi-transport against real loro-repo', () => {
  beforeEach(() => {
    ...
    process.env.MOLLY_LORO_WAIT_META_SYNC_TIMEOUT_MS = '60000';
    process.env.MOLLY_LORO_AUTO_RECONNECT_INTERVAL_MS = '0';
  });

  afterEach(() => {
    if (originalWaitMetaTimeoutEnv === undefined) {
      delete process.env.MOLLY_LORO_WAIT_META_SYNC_TIMEOUT_MS;
    } else {
      process.env.MOLLY_LORO_WAIT_META_SYNC_TIMEOUT_MS = originalWaitMetaTimeoutEnv;
    }
