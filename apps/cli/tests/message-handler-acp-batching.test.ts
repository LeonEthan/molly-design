...
type StageSessionFileLocallyForTest = (args: {
  sessionId: SessionId;
  file: {
    absolutePath: string;
    fileName: string;
...
describe('MessageHandler ACP batching', () => {
  ...
  it('stores ACP image content as an inline local attachment without cloud I/O', async () => {
    const sessionId = 's-1' as SessionId;
    const imageBytes = Buffer.from('fake-png-bytes');
    const sha256 = crypto.createHash('sha256').update(imageBytes).digest('hex');
    vi.stubGlobal('fetch', () => {
      throw new Error('Unexpected product-cloud request');
    });
    const { repo, docs, handler } = await createHandlerHarness([sessionId]);
    const doc = docs.get(sessionId);
    if (!doc) throw new Error(`Missing session doc for ${sessionId}`);
    vi.spyOn(
      handler as unknown as { stageSessionFileLocally: StageSessionFileLocallyForTest },
      'stageSessionFileLocally'
    ).mockResolvedValue({
      type: 'file',
      fileId: 'file-image',
