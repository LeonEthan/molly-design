...
describe('MessageHandler session file ACP prompt blocks', () => {
  ...
  it('accepts a reserved local session, reopens its sent blob by identity, and rejects a mismatched hash', async () => {
    const previous = process.env.MOLLY_DATA_DIR;
    process.env.MOLLY_DATA_DIR = path.join(tmpDir, 'data');
    ...
    try {
      ...
      if (previous === undefined) delete process.env.MOLLY_DATA_DIR;
      else process.env.MOLLY_DATA_DIR = previous;
    }
  });
