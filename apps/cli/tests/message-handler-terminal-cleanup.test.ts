...
describe('MessageHandler terminal cleanup', () => {
  ...
  it('leaves a dirty session worktree untouched when removing its local project', async () => {
    ...
    const originalLocksDir = process.env.MOLLY_LOCKS_DIR;
    try {
      process.env.MOLLY_DATA_DIR = path.join(testDir, 'data');
      ...
      if (originalLocksDir === undefined) delete process.env.MOLLY_LOCKS_DIR;
      else process.env.MOLLY_LOCKS_DIR = originalLocksDir;
