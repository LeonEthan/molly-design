...
import { ACP_CAPABILITY_CACHE_VERSION } from '@molly/shared';
...
vi.mock('@/pkg', () => ({
  // Both named (instrument.ts) and default (analytics poster / molly-fleet) importers
...
describe('Lody.registerAgent', () => {
  ...
  it('starts builtin registration when the local runtime starts', async () => {
    const { Molly } = await import('../src/lib/molly');
    const documentManager = {
      ensureMachineFlockDocJoined: vi.fn(),
    } as unknown as LoroDocumentManager;
    const lody = new Molly(
      {
    ...
    const registeredCliTypes: string[][] = [];
    vi.spyOn(lody, 'registerAgent').mockImplementation(async (cliTypes) => {
      registeredCliTypes.push(cliTypes);
    });

    expect(registeredCliTypes).toEqual([]);

    await lody.start();
    await flushMicrotasks();

    expect(registeredCliTypes).toEqual([['codex']]);
