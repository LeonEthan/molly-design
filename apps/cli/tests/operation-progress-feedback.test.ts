...
import { MollyOperationCoordinator } from '../src/orchestration/operation-coordinator';
...
describe('nested operation progress feedback', () => {
  it('quiesces for A -> B -> C and still publishes subsequent child completion', async () => {
    ...
    const operation = (parent: string, child: string): StoredMollyOperation => ({
    ...
    let scans = 0;
    let coordinator: MollyOperationCoordinator;
    ...
    coordinator = new MollyOperationCoordinator({
