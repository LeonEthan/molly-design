...
import { createSessionMirror, sessionDocSchema } from '@molly/shared';
...
const baseOperation = (items: MollyOperationItemResult[]): StoredMollyOperation => ({
  workspaceId: 'workspace-1' as StoredMollyOperation['workspaceId'],
  ownerMachineId: 'machine-1' as StoredMollyOperation['ownerMachineId'],
...
it.each(['failed', 'cancelled'] as const)(
  ...
  async (status) => {
    ...
    const terminal: MollyOperationItemResult =
      status === 'failed'
        ? {
            status,
            target,
  ...
  async (terminalStatus) => {
    ...
    try {
      for (const status of ['created', 'running', terminalStatus] as const) {
        const item: MollyOperationItemResult =
          status === 'succeeded'
