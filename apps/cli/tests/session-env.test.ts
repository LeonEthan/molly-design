...
describe('Session buildShellEnv', () => {
  ...
  it('exposes the workspace owner session id for child sessions', () => {
    ...
    expect(env.MOLLY_SESSION_ID).toBe('child-session-2');
    expect(env.MOLLY_WORKSPACE_SESSION_ID).toBe('parent-session-1');
  });
  ...
  it('falls back to its own session id when no parent session exists', () => {
    ...
    expect(env.MOLLY_SESSION_ID).toBe('session-1');
    expect(env.MOLLY_WORKSPACE_SESSION_ID).toBe('session-1');
  });
  ...
  it('allows updateEnv to remove stale session environment keys', () => {
    const session = new Session(
      createConfig({
        env: {
          GH_TOKEN: 'old-token',
          MOLLY_MANAGED_GH_TOKEN_SHA256: 'old-marker',
        },
      }),
      createSilentLogger()
    );
    ...
    session.updateEnv({
      GH_TOKEN: undefined,
      MOLLY_MANAGED_GH_TOKEN_SHA256: undefined,
      MOLLY_GIT_CRED_CONTEXT_TOKEN: 'next-context',
    });

    const env = (session as unknown as { buildShellEnv(): NodeJS.ProcessEnv }).buildShellEnv();

    expect(env.GH_TOKEN).toBeUndefined();
    expect(env.MOLLY_MANAGED_GH_TOKEN_SHA256).toBeUndefined();
    expect(env.MOLLY_GIT_CRED_CONTEXT_TOKEN).toBe('next-context');
  });
