...
import type { SessionId, WorkspaceId, WorktreeSetupScriptConfig } from '@molly/shared';
...
describe('worktree setup runner', () => {
  it('runs script lines in one shell while emitting per-step output', async () => {
    ...
    const script =
      shell === 'powershell'
        ? "$env:MOLLY_TEST_VALUE = 'hello'\nWrite-Output $env:MOLLY_TEST_VALUE\n[Console]::Error.WriteLine('err')"
        : 'export MOLLY_TEST_VALUE=hello\nprintf "$MOLLY_TEST_VALUE\\n"\nprintf \'err\\n\' >&2';
