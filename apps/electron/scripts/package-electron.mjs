...
// which carries Molly's independent release line (0.1.0 onward; the inherited
...
function resolveVersion(forwardedArguments) {
  ...
  const candidate =
    flagIndex === -1
      ? (process.env.MOLLY_OSS_RELEASE_VERSION ?? process.env.LODY_OSS_RELEASE_VERSION ?? '')
  ...
  if (!SEMVER_PATTERN.test(version)) {
    throw new Error(
      `Refusing to package with version ${JSON.stringify(version)}. Pass --version=<semver>, ` +
        'set MOLLY_OSS_RELEASE_VERSION, or fix apps/electron/package.json.'
    )
  }
  return version
}
...
const generatedConfigDirectory = mkdtempSync(path.join(tmpdir(), 'molly-electron-builder-'))
...
console.log(`[package-electron] packaging Molly ${version}`)
