...
import type { SessionId, WorkspaceId } from '@molly/shared/ids'
import { SessionIdSchema } from '@molly/shared/message-schemas'
import type { LocalProjectHistoryProvider, LocalProjectId } from '@molly/shared/project'
...
export class LocalProjectsIpc extends IpcService {
  ...
  async sendSessionFileLocal(
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'molly-file-send-'))
  ...
  async selectDirectory() {
    ...
    if (!machineId) {
      return {
        error: 'The Molly background service is unavailable. Restart Molly and try again.'
      }
    }
