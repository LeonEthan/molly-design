...
import { LocalFileResolutionSchema } from '@molly/shared/local-file-preview'
import { localFileResources } from '../../services/local-file-resource-protocol'
import { getIpcContext, IpcMethod, IpcService } from 'electron-ipc-decorator'
import type { LocalMachineRpcRequest } from '@molly/shared/local-machine-rpc'
...
export class MachineRpcIpc extends IpcService {
  ...
  async previewFile(message: LocalMachineRpcRequest) {
    ...
    const response = await localFileResources.preview(
      generation,
      file.data,
      'attachment' in parsed.params
    )
    if (owner.isDestroyed() || resourceOwners.get(owner.id) !== generation) {
