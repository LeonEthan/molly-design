...
void test('claims the privileged path only for a downloaded .deb on Linux', () => {
  assert.deepEqual(
    resolveLinuxDebInstallPlan({
      platform: 'linux',
      downloadedFile: '/home/u/.cache/pending/Molly-0.91.1-amd64.deb',
    ...
    }),
    {
      ...
      args: [
        ...
        'dpkg',
        '-i',
        '/home/u/.cache/pending/Molly-0.91.1-amd64.deb'
      ]
    }
  )
})
...
void test('passes a package path with spaces as one unescaped argument', () => {
  const plan = resolveLinuxDebInstallPlan({
    platform: 'linux',
    downloadedFile: '/home/my user/pending/Molly 0.91.1.deb',
    appImagePath: undefined
  })
  assert.deepEqual(plan?.args.at(-1), '/home/my user/pending/Molly 0.91.1.deb')
})
...
void test('leaves every other install to electron-updater', () => {
  ...
  assert.equal(
    resolveLinuxDebInstallPlan({
      platform: 'linux',
      downloadedFile: '/tmp/Molly-0.91.1-x86_64.AppImage',
      appImagePath: undefined
    }),
    null
  )
