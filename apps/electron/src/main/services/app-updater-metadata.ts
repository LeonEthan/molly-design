import type { ElectronUpdaterState } from '@molly/shared/electron-ipc'
...
> {
  ...
  const mollyChangelog = readObject(vendor?.mollyChangelog ?? vendor?.lodyChangelog)
  const contentVersion = readNonEmptyString(mollyChangelog?.contentVersion)
  const releaseNotesByLocale =
    expectedVersion && contentVersion === expectedVersion
      ? readReleaseNotesByLocale(mollyChangelog?.locales)
      : undefined
