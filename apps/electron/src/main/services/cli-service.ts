...
import { isLocalSessionControlRequest } from '@molly/shared/node/local-session-control'
import { isLocalProjectControlRequest } from '@molly/shared/node/local-project-control'
import {
...
import { ACP_CAPABILITIES_REFRESH_CLIENT_BACKSTOP_MS } from '@molly/shared/acp-startup-budget'
import type {
  LocalProjectControlRequest,
  LocalProjectControlResponse,
  LocalSessionControlRequest,
  LocalSessionControlResponse
} from '@molly/shared/message'
import type {
  ElectronCliState,
  RestartCliResult,
...
const CLI_ELECTRON_BOOTSTRAP_ENV = 'MOLLY_ELECTRON_BOOTSTRAP'
...
const MOLLY_DATA_DIR = getMollyDataDir(mainPlatformKind)
...
const CLI_CREDENTIALS_PATH = join(MOLLY_DATA_DIR, 'credentials.json')
...
function buildCliRuntimeEnvOverrides(): NodeJS.ProcessEnv {
  return { MOLLY_PLATFORM: mainPlatformKind, MOLLY_DATA_DIR }
}
...
function writeElectronSettings(settings: Record<string, unknown>): void {
  try {
    if (!fs.existsSync(MOLLY_DATA_DIR)) {
      fs.mkdirSync(MOLLY_DATA_DIR, { recursive: true })
    }
...
export class CliService {
  ...
  constructor() {
    ...
    if (Object.hasOwn(settings, 'cliAutoStartEnabled')) {
      delete settings.cliAutoStartEnabled
  ...
  private async buildLaunchPreparation(signal: AbortSignal): Promise<PreparedLaunch> {
    ...
    const envOverrides: NodeJS.ProcessEnv = {
      ...
      [MOLLY_SUPERVISOR_INSTANCE_ID_ENV]: this.supervisorInstanceId,
      [MOLLY_SUPERVISOR_TOKEN_ENV]: this.supervisorToken
    }
