...
import { getImageMimeTypeForPath, isBinaryImagePath } from '@molly/shared/image-file-types'
import type {
  LocalFileResolution,
  LocalFilePreviewResource
} from '@molly/shared/local-file-preview'
import { LOCAL_TEXT_EDIT_BYTES, LOCAL_TEXT_PAGE_BYTES } from '@molly/shared/local-file-preview'
...
export class LocalFileResources {
  ...
  async preview(owner: number, file: LocalFileResolution, resourceOnly = false) {
    ...
    try {
      ...
      if (!resourceOnly && text !== undefined && stat.size <= LOCAL_TEXT_EDIT_BYTES) {
        return {
          status: 'ok' as const,
          v: 3 as const,
          path: file.path,
