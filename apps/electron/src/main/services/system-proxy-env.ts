 ...
 * that read the uppercase spelling first then proxied Molly's own loopback
 * services. See `@molly/shared/proxy-env`.
 */
...
export async function resolveSystemProxyEnv(
  ...
  if (
    (process.env.MOLLY_ELECTRON_DISABLE_SYSTEM_PROXY_ENV ??
      process.env.LODY_ELECTRON_DISABLE_SYSTEM_PROXY_ENV) === '1'
  ) {
    return {}
  }
