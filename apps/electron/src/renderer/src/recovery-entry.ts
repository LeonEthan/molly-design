import { renderBootFailure } from '@molly/components/lib/boot-failure'
...
const message = params.get('message') ?? 'Molly could not load the main window.'
...
renderBootFailure(rootElement, reconstructed, {
  buildInfo,
  hint: 'Molly could not load the main window. Click Reload to try again, or Copy error and share it with the team.',
  onReload: () => {
    if (getIpcServices()) {
      try {
        ...
        console.warn('[Molly] requestRendererReload bridge threw', error)
      }
    }
  ...
  onCopy: () => {
    try {
      ...
      console.warn('[Molly] reportRendererFatalError bridge threw', error)
    }
  }
})
