...
describe('formatCommandForDisplay', () => {
  it('quotes every POSIX token so command and entry paths are visually distinct', () => {
    expect(
      formatCommandForDisplay(
        '/Applications/Lody.app/Contents/MacOS/Molly',
        [
      ...
      "'/Applications/Lody.app/Contents/MacOS/Molly' '/Applications/Lody.app/Contents/Resources/app.asar.unpacked/resources/cli/index.js' 'start'"
    );
  });
  ...
  it('uses Windows-style quoting on win32', () => {
    expect(
      formatCommandForDisplay('C:\\Program Files\\Molly\\lody.exe', ['say "hi"'], 'win32')
    ).toBe('"C:\\Program Files\\Molly\\lody.exe" "say \\"hi\\""');
  });
});
