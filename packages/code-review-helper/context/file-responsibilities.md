# @molly/code-review-helper — File Responsibilities
...
- `src/file-icons/` holds per-extension file-type icons (`files/*.svg` + `mappings.ts`
  - `FileIcon`), copied from `@molly/components` (MIT vscode-symbols) so the package
...
- `src/ui/` contains package-local shadcn components. Do not import `@molly/components`
...
- `src/standalone/` is the single-file viewer: `main.tsx` (entry; reads
  `window.__MOLLY_REVIEW__`), `inject.ts` (`injectReviewSnapshot`, exported via
