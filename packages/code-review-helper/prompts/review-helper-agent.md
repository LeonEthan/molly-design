...
3. Determine `current_commit`:
   - For a PR target, FETCH the PR branch locally first (e.g. `gh pr checkout <number>`
     or `git fetch <remote> <ref>`), then use its head commit. `molly review` renders by
...
path you wrote it to. (`molly review` resolves line references against the repo via
...
`.review.html` (printed by `molly review`) — so they can re-open, move, or share them.
