...
} from '@molly/components/lib/vscode-theme';

type CodeReviewThemeContextValue = {
  readonly activeTheme: MollyResolvedVSCodeTheme | undefined;
 ...
 *
 * This mirrors the `@molly/components` VSCode theme adaptation so the review
 * helper looks consistent with the rest of Molly when running standalone.
 */
export function CodeReviewThemeProvider({ children }: { readonly children: ReactNode }) {
  ...
  useLayoutEffect(() => {
    ...
    void registerMollyVSCodeThemeForDiffs(theme).catch((error: unknown) => {
      if (!cancelled) {
  ...
  const value = useMemo<CodeReviewThemeContextValue>(
    () => ({
      activeTheme: theme,
      diffThemeName: theme ? createMollyVSCodeShikiThemeName(theme) : FALLBACK_DIFF_THEME_NAME,
    }),
    [theme]
  );
