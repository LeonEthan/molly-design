 ...
 * data. Consumers (the package CLI and `molly review`) call `injectReviewSnapshot`
...
export const REVIEW_GLOBAL_NAME = '__MOLLY_REVIEW__';

const PLACEHOLDER = '<!--MOLLY_REVIEW_DATA-->';

/**
 * Serializes `data` to a `<script>` that assigns `window.__MOLLY_REVIEW__` and
 ...
 * never assigns `window.__MOLLY_REVIEW__`, so the viewer renders "No review data
