# Standalone single-file viewer (`molly review`)

The standalone build produces ONE self-contained HTML file that renders a review
over `file://` with no server. It backs `molly review <file>.review.md` (CLI) and
...
1. `vite build --config vite.standalone.config.ts` bundles `standalone.html` +
   ...
   has a `<!--MOLLY_REVIEW_DATA-->` placeholder in `<head>`).
...
cleanly in both; the CLI lazy-imports `@molly/code-review-helper/prompt-text` inside the
command action.
...
`src/standalone/main.tsx` reads `window.__MOLLY_REVIEW__` and renders `ReviewRenderer`
