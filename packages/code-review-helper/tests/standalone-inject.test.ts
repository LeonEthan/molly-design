...
const TEMPLATE_WITH_PLACEHOLDER =
  '<!doctype html><html><head><title>x</title><!--MOLLY_REVIEW_DATA--></head><body><div id="root"></div><script type="module">app()</script></body></html>';
...
describe('injectReviewSnapshot', () => {
  it('replaces the placeholder with a global-assignment script', () => {
    ...
    expect(html).not.toContain('<!--MOLLY_REVIEW_DATA-->');
  ...
  it('does not expand $ replacement patterns from review data (placeholder path)', () => {
    ...
    expect(injected).not.toContain('<!--MOLLY_REVIEW_DATA-->');
