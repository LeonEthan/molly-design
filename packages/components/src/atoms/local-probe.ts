...
import type { ElectronCliState, MachineId } from '@molly/shared';
 ...
 * `apps/cli/src/lib/molly-fleet.ts`). Firing control requests before then makes
...
function isElectronRenderer(): boolean {
  return hasBrowserWindow() && window.__MOLLY_ELECTRON__ === true;
}

function getElectronHomeDir(): string | null {
  return isElectronRenderer() ? (window.__MOLLY_PLATFORM__?.homeDir ?? null) : null;
}
