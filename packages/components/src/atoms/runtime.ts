...
export type WorkspaceRuntime = {
  ...
  subscribeMachineMonitor: (
    machineId: MachineId,
    listener: (snapshot: import('@molly/shared').MachineMonitorSnapshot | null) => void
  ) => () => void;
  ...
  requestSessionTerminate: (
    ...
    sessionId: SessionId,
    options?: { timeoutMs?: number }
  ) => Promise<import('@molly/shared').SessionTerminateResponse | null>;
  requestSessionFork: (
    machineId: MachineId,
  ...
  requestSessionEditAndResend: (
    ...
    options?: { timeoutMs?: number }
  ) => Promise<import('@molly/shared').SessionEditAndResendResponse | null>;
...
// Local desktop never bootstraps a product-cloud token, including from older
// Molly localStorage keys left on the same machine.
