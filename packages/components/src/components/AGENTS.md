...
- Desktop working/waiting/unread status belongs only in `SessionRowStatusIndicator`
  ...
  slot. Status replaces resting line diff, `Mergeable`, worktree glyph, or PR icon
  with one 14px mark; retain metrics in the desktop hover info card.
...
- Collapse state uses `sidebarCollapsedOpenedBySessionsAtom`, default expanded. Keep
  ...
  successful create Operations / the precise opener.
...
- `web-workspace-layout.tsx` owns top/side safe-area insets for desktop surfaces.
  The bottom inset belongs to the adjacent surface (the composer uses
