...
export type ChatShareCardBackdrop = 'none' | 'molly' | 'aurora' | 'ocean' | 'sunset' | 'welcome';
...
export interface ChatShareCardProps {
  ...
  showQr?: boolean;
  /**
   * Pins the card to one of the bundled Molly palettes (lody-light / vesper)
  ...
  /** EXIF-style caption for the `exif` footer: a bold device/model line and a
      ...
      replaces the Molly mark — e.g. pass an `AgentIcon` for the driving agent. */
 ...
 * a backdrop. The footer carries the Molly brand and an optional QR code.
 */
  ...
  const card = (
    ...
    >
      {wrap ? (
        <style>
          {
            '.molly-chat-share-card-wrap .markdown-renderer [data-streamdown=code-block-body] pre { min-width: 0; white-space: pre-wrap; overflow-wrap: anywhere; }'
          }
        </style>
      ) : null}
      ...
      <div
        ref={contentRef}
        className={cn('px-6 pt-4 pb-5 space-y-4', wrap && 'molly-chat-share-card-wrap')}
      >
