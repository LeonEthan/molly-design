...
import { getMollyDocumentationUrl, MOLLY_ISSUES_URL } from '@/lib/molly-urls';
...
import { getChangelogUrl } from '@/lib/molly-urls';
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>
            {t('sidebar.localProjects.remove.title', 'Remove “{{name}}” from Molly?', {
              name: target?.name ?? '',
            })}
          </DialogTitle>
          <DialogDescription>
          ...
          <div className="rounded-lg bg-muted/60 px-3.5 py-3">
            <p className="font-medium text-foreground/90">
              {t(
                ...
                'Molly never deletes the original project folder or its files.'
              )}
            </p>
              ...
              <span>
                <span className="block font-medium text-foreground">
                  {t(
                    ...
                    'Also delete session worktrees created by Molly'
                  )}
                </span>
...
const isElectronMacOS =
  ...
  window.__MOLLY_PLATFORM__?.os === 'darwin';
  ...
  const menuRow = hasContextMenuActions ? (
    <ContextMenu onOpenChange={setRowMenuOpen}>
      ...
      <ContextMenuContent className="min-w-[180px]">
        ...
        {canCopyUrl ? <ContextMenuSeparator /> : null}
        {canCopyUrl ? (
          <ContextMenuItem
            icon={<Link2 />}
            onSelect={() => {
...
export function LoroAppSidebar({ className }: LoroAppSidebarProps) {
  ...
  const connectionUiState = useAtomValue(mollyConnectionUiStateAtom);
  ...
  const [dismissedUpdaterVersion, setDismissedUpdaterVersion] = useState<string | null>(() => {
    try {
      return mollyStorage.getItem('molly:dismissedUpdaterVersion');
    } catch {
      return null;
    }
  });
  ...
  const { machines: machineMetaMap, projects: visibleLocalProjectMap } = useLocalResourceVisibility(
    {
  ...
  // Include every local session, including historical records with older author IDs.
  const scope: SessionListScope = 'team';
  ...
  const localProjectSessionsByKey = useMemo(() => {
    ...
    return map;
  }, [childSessionsByParent, sessions, sessionsListLoading]);
  ...
  }, [multiWorkspaceAvailable, organizations]);
  ...
  const handleDismissUpdateBanner = useCallback(() => {
    ...
    try {
      mollyStorage.setItem('molly:dismissedUpdaterVersion', updateBanner.version);
    } catch {
  ...
  const handleWorkspaceSelected = useCallback(
    ...
    [organizations, router, setWorkspaceContext, switchOrganization]
  );
