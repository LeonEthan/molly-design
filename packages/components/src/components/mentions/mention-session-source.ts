 ...
 */
const SLUG_CACHE_KEY = 'molly:session-mention-slugs';
...
function readSlugCache(): SlugCache {
  ...
  try {
    return parseSlugCache(mollyStorage.getItem(SLUG_CACHE_KEY));
  } catch {
    return {};
  }
}
...
export function rememberSessionMentionSlugs(items: readonly SessionMentionItem[]): void {
  ...
  try {
    const raw = mollyStorage.getItem(SLUG_CACHE_KEY);
    ...
    mollyStorage.setItem(SLUG_CACHE_KEY, serialized);
  } catch {
...
export function buildSessionMentionPrompt(sessionId: string): string {
  return `use molly mcp to query session[id: ${sessionId}] history`;
}
