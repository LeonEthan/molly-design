...
/**
 * Always-visible brands beneath the Add button — makes it obvious Molly runs far
  ...
  return (
    <OnboardingShell
      ...
      description={t(
        ...
        'Add an Agent now. Molly will continue setup and let you know if anything needs your attention.'
      )}
      previewIdentity={
        previewConfig
          ? {
...
/**
 * "Logo wall" of supported ACP agents. Communicates that Molly runs far more
        ...
        if (!restart) {
          ...
          toast.error(
            t(
              ...
              'Could not reach the local agent. Please restart Molly and try again.'
            )
          );
          return;
        }
...
function ProviderStatusBadge({
  ...
  if (activity) {
    ...
    const slowDetail = t(
      ...
      'Agent setup is still running. A first run may have to download and unpack the agent. You can continue — Molly keeps working on this in the background.'
    );
    const badge = (
      <Badge
        variant="outline"
