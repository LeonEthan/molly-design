...
import type { GitHubMergeMethod } from '@molly/shared';

const MERGE_METHOD_STORAGE_KEY = 'molly:pr-tab:last-merge-method';
...
function readStoredMergeMethod(): GitHubMergeMethod {
  ...
  try {
    const raw = mollyStorage.getItem(MERGE_METHOD_STORAGE_KEY);
...
export function setPreferredPrMergeMethod(method: GitHubMergeMethod): void {
  ...
  try {
    if (typeof window !== 'undefined') {
      mollyStorage.setItem(MERGE_METHOD_STORAGE_KEY, method);
    }
  } catch {
