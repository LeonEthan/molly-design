...
import { SESSION_FILE_MAX_COUNT, SESSION_IMAGE_MAX_SIZE_BYTES } from '@molly/shared';
import type { SessionFilePayload, AgentConfigMeta } from '@molly/shared';
import {
...
export const SessionChatInputArea = memo(
  forwardRef<SessionChatInputAreaHandle, SessionChatInputAreaProps>(function SessionChatInputArea(
    ...
    const startFileUpload = useCallback(
      async (targetSessionId: SessionId, localId: string, file: File) => {
        if (!workspaceId || !canSendFileLocally || !session.machineId) {
        ...
        try {
          ...
          if (!outcome?.ok || !outcome.files[0]) throw new Error(fileUploadFailedLabel);
          updatePendingFile(targetSessionId, localId, (entry) => ({
            ...entry,
            status: 'uploaded',
            progress: 100,
    ...
    const sessionLocalFileSource = useMemo(
      () =>
        resolveSessionLocalFileSource(session, {
          isElectronRenderer: typeof window !== 'undefined' && window.__MOLLY_ELECTRON__ === true,
          localMachineId,
