...
import { MOLLY_DISCORD_URL } from '@/lib/molly-urls';
...
export function JoinCommunityDialog({
  ...
  const handleJoinDiscord = () => {
    void openExternalUrl(MOLLY_DISCORD_URL);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xl">
        <DialogHeader>
          <DialogTitle>
            {t('settings.about.communityDialogTitle', 'Join the Molly community')}
          </DialogTitle>
          <DialogDescription>
            {t(
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="flex flex-col items-center gap-3 rounded-lg border border-border/70 bg-card/70 p-4">
            ...
            <img
              src={qrImageUrl}
              alt={t('settings.about.feishuGroupQrAlt', 'Molly Feishu group QR code')}
