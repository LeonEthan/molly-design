...
type ProjectSettingsSelection = { key: string; row: ProjectSettingsRow };
...
export function ProjectSettingsView(props: ProjectSettingsViewProps) {
  return <ProjectSettingsDesktop {...props} />;
}
...
function ProjectSettingsDesktop({
  ...
  const totalCount = totalProjects;
  ...
  // The selected machine drives the project list.
  ...
  const currentSelections = useMemo<ProjectSettingsSelection[]>(() => {
    ...
    return (section?.rows ?? []).map((row) => ({ key: row.key, row }));
  }, [resolvedPillId, sections]);
...
function ProjectDetailPane({
  ...
  return <LocalProjectDetail row={selection.row} {...handlers} />;
}
