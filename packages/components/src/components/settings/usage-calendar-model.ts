...
/** Parse the command subset used by the checked-in Molly SVG into printable contours. */
export function parseMollyLogoContours(): Point[][] {
  const path = lodySvg.match(/\bd="([^"]+)"/u)?.[1];
  if (!path) throw new Error('Molly SVG path is unavailable');
  ...
  while (tokenIndex < tokens.length) {
    ...
    throw new Error(`Unsupported Molly SVG command: ${command}`);
  }
...
function appendMollyLogoRelief(triangles: Triangle[]) {
  ...
  for (const contour of parseMollyLogoContours()) {
...
function appendMollyWordmarkRelief(triangles: Triangle[]) {
  const cellSize = 0.68;
  const glyphGap = 0.42;
  ...
  for (const glyph of 'MOLLY') {
    const rows = MOLLY_WORDMARK_GLYPHS[glyph as keyof typeof MOLLY_WORDMARK_GLYPHS];
...
export function createUsageSkylineMollyLogoTriangles(): UsageSkylineStlTriangle[] {
  const triangles: UsageSkylineStlTriangle[] = [];
  appendMollyLogoRelief(triangles);
  appendMollyWordmarkRelief(triangles);
  return triangles;
}
...
export function createUsageSkylineBinaryStl(model: UsageCalendarModel): ArrayBuffer {
  ...
  const header = new TextEncoder().encode('Molly usage skyline binary STL');
