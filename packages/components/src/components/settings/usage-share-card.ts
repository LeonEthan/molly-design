...
export type MollyMarkStyle = 'sticker' | 'outline' | 'plain';
...
export type UsageShareCardConfig = {
  ...
  // Molly sticker on the main body
  showMark: boolean;
  markStyle: MollyMarkStyle;
...
const lodyContours = parseMollyLogoContours();
const lodyBounds = (() => {
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
...
function traceMollyPath(context: CanvasRenderingContext2D, scale: number) {
...
/** Draw the Molly glyph as a die-cut outline sticker centered on (x, y). */
export function drawMollyMark(
...
function drawMark(
  ...
  drawMollyMark(context, markX, cfg.markY, cfg.markSize, {
...
function drawStub(
  ...
  if (cfg.showStubStamp) {
    ...
    // Branded centre badge with the Molly mark (never glitched here).
    ...
    context.fill();
    context.restore();
    drawMollyMark(context, centerX, qrY + qrSize / 2, badge * 0.74, {
      style: 'plain',
      fill: cfg.accent,
