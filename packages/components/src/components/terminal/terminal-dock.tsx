...
export function TerminalDock({
  ...
  useEffect(() => {
    ...
    window.dispatchEvent(
      new CustomEvent('molly:terminal-dock-resize', { detail: { open: isOpen } })
    );
  }, [isOpen]);
      ...
      {isOpen ? (
        ...
        >
          ...
          <div className="flex h-8 flex-none items-center gap-3 border-b border-sidebar-border/60 px-2 text-[11px] text-muted-foreground">
            <div className="molly-terminal-tab-strip flex h-full min-w-0 flex-1 items-center gap-3 overflow-x-auto">
              {sessionId
                ? terminals.map((term) => {
