...
import { type AgentConfigId, type MachineId, getAgentConfigRoomId } from '@molly/shared';
...
const MUTEX_KEY_PREFIX = 'molly-agent-config-migration:';
...
function acquireMutex(key: string): boolean {
  ...
  try {
    ...
    mollyStorage.setItem(key, String(now));
    return true;
  } catch {
    return true;
  }
