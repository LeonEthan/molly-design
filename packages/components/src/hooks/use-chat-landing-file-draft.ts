...
import { machineSupportsLocalSessionAttachments } from '@molly/shared';
 ...
 *
 * Hands bytes directly to the local CLI. Failed handoff stays visible as an
 * attachment error and can be retried by the user.
 */
export function useChatLandingFileDraft(args: {
  ...
  // The local CLI and Electron preload bridge must support attachment handoff.
  ...
  const fileUploadMissingAuthLabel = t(
    ...
    'Local workspace or attachment runtime is unavailable'
  );
  ...
  const startUpload = useCallback(
    async (localId: string, file: File, sessionId: SessionId) => {
      if (!workspaceId || !canSendFileLocally || !machineId) {
      ...
      try {
        ...
        if (!outcome?.ok || !outcome.files[0]) {
          throw new Error(fileUploadFailedLabel);
        }
        updatePendingFile(localId, (entry) => ({
          ...entry,
          status: 'uploaded',
        ...
        updatePendingFile(localId, (entry) => ({
          ...
          status: 'failed',
          progress: 0,
          error: error instanceof Error ? error.message : fileUploadFailedLabel,
        }));
      }
    },
    [canSendFileLocally, fileUploadFailedLabel, fileUploadMissingAuthLabel, machineId, updatePendingFile, workspaceId]
  );
