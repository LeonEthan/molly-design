...
/* Shared backing store for both the desktop `/settings/projects` page
   for the local desktop project page and machine resource pane. */
...
export function useLocalProjectsAdmin(): LocalProjectsAdminData {
  ...
  const { projects, isLoading } = useVisibleLocalProjects();
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && window.__MOLLY_ELECTRON__ && !localMachineId) return;
