...
import type { SupportedLanguage } from '@molly/shared';
...
const LANGUAGE_STORAGE_KEY = 'molly-language';
...
export function readStoredLanguagePreference(): SupportedLanguage | null {
  ...
  try {
    return parseStoredLanguageValue(mollyStorage.getItem(LANGUAGE_STORAGE_KEY));
  } catch {
    return null;
  }
}
...
export function detectBrowserLanguage(): SupportedLanguage | null {
  ...
  const candidates =
    window.__MOLLY_ELECTRON__ === true
      ? (window.__MOLLY_PLATFORM__?.preferredSystemLanguages ?? [])
