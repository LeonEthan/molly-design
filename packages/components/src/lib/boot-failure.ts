...
const BOOT_BEACON_INSTALL_ID_KEY = 'molly_install_id';
...
function resolveBootBeaconInstallId(): string {
  ...
  try {
    const existing = mollyStorage.getItem(BOOT_BEACON_INSTALL_ID_KEY);
    if (existing) return existing;
    const generated =
      typeof crypto !== 'undefined' && typeof crypto.randomUUID === 'function'
        ? crypto.randomUUID()
        : `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 10)}`;
    mollyStorage.setItem(BOOT_BEACON_INSTALL_ID_KEY, generated);
    return generated;
  } catch {
    return 'anonymous';
  }
...
// Boot diagnostics stay on this machine in the public desktop build.
function postBootBeaconEvent(_eventName: string, _properties: Record<string, unknown>): void {}
...
export function renderBootFailure(
  ...
  console.error('[Molly] Boot failed.\n\nError: %s\n\nDiagnostics:\n%s', diag.message, diag.details);
  ...
  const cardHtml =
    ...
    `<div style="${TITLE_STYLE}">Molly could not start</div>` +
      ...
      try {
        ...
        if (result && typeof (result as Promise<void>).then === 'function') {
          (result as Promise<void>)
            .catch((reloadErr) => {
              console.error('[Molly] onReload threw, falling back to location.reload', reloadErr);
