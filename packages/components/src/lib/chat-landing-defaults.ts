...
import type { AgentConfigId, AgentConfigMeta, MachineId, MachineViewMeta } from '@molly/shared';
import { z } from 'zod';
...
export const CHAT_LANDING_DEFAULTS_KEY = 'molly:chatLandingDefaults';
...
export function readChatLandingDefaults(
  ...
  try {
    const raw = mollyStorage.getItem(getChatLandingDefaultsStorageKey(workspaceId));
    if (!raw) return null;
...
export function writeChatLandingDefaults(
  ...
  try {
    mollyStorage.setItem(getChatLandingDefaultsStorageKey(workspaceId), JSON.stringify(defaults));
  } catch {
    // ignore
  }
}
