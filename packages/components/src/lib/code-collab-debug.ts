...
export const CODE_COLLAB_DEBUG_STORAGE_KEY = 'molly:debug:code-collab';
...
export function isCodeCollabDebugEnabled(): boolean {
  ...
  try {
    return isEnabledDebugValue(mollyStorage.getItem(CODE_COLLAB_DEBUG_STORAGE_KEY));
  } catch {
    return false;
  }
}
...
function getCodeCollabTraceBuffer(): Array<{
  ...
  target.__mollyCodeCollabTrace ??= [];
  return target.__mollyCodeCollabTrace;
}
