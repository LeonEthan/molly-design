...
import type { AgentConfigCliType, AgentConfigMeta } from '@molly/shared';
 ...
 * exactly the providers whose subscription usage Molly already reports: a
...
export const fetchCodexResetStatus: CodexResetStatusFetcher = async ({ etag, signal }) => {
  const response = await fetch(CODEX_RESETS_STATUS_URL, {
    method: 'GET',
    // Third-party public endpoint: never attach Molly credentials to it.
    credentials: 'omit',
    headers: {
