import { GLOBAL_SHORTCUT_DEFAULTS, type GlobalShortcutId } from '@molly/shared';
...
const whileComposerFocused = (key: string): KeyBinding => ({
  ...
  when: (event) => {
    ...
    return target instanceof Element && target.closest('[data-molly-composer-input]') !== null;
  },
});
 ...
 * app-wide even when Molly isn't focused, so they can't be a normal registry command
 ...
 * intentionally unbound `null`) live in `@molly/shared`'s `GLOBAL_SHORTCUT_DEFAULTS`; the
 * user's live binding is read/written over IPC via `useGlobalShortcuts`. To add another
 * global shortcut, see the checklist on `GlobalShortcutId` in `@molly/shared/electron-ipc`.
 */
...
export const GLOBAL_SHORTCUTS: GlobalShortcut[] = [
  {
    ...
    defaultTitle: 'Bring Molly to Front',
