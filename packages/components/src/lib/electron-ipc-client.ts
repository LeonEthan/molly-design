...
import type { LocalSessionControlRequest, LocalSessionControlResponse } from '@molly/shared';
import { LocalSessionControlResponseSchema } from '@molly/shared/message-schemas';
...
export type MollyIpcInvokeBridge = {
  invoke: (channel: string, ...args: unknown[]) => Promise<unknown>;
};

export function createMollyIpcProxy<T extends Record<string, object> = IpcServices>(
  ipc: MollyIpcInvokeBridge | null | undefined
): T | null {
  if (!ipc) return null;
...
function readIpcBridge(): MollyIpcInvokeBridge | null {
  if (typeof window === 'undefined') return null;
  return window.ipc ?? null;
}

export function getIpcServices(): IpcServices | null {
  return createMollyIpcProxy<IpcServices>(readIpcBridge());
}
