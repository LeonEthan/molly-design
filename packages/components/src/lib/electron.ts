...
export const isElectronRenderer = (): boolean => {
  return typeof window !== 'undefined' && window.__MOLLY_ELECTRON__ === true;
};

export const isMacOSElectronRenderer = (): boolean => {
  return isElectronRenderer() && window.__MOLLY_PLATFORM__?.os === 'darwin';
};

export const isWindowsElectronRenderer = (): boolean => {
  return isElectronRenderer() && window.__MOLLY_PLATFORM__?.os === 'win32';
};
