...
const LAST_APP_ROUTE_STORAGE_KEY = 'molly:lastAppRoute';
...
export function readLastAppRoutePath(): string | null {
  ...
  try {
    const raw = mollyStorage.getItem(LAST_APP_ROUTE_STORAGE_KEY);
    if (!raw) {
      return null;
    }
    ...
    if (!isStoredLastAppRoute(parsed)) {
      mollyStorage.removeItem(LAST_APP_ROUTE_STORAGE_KEY);
      return null;
    }
...
export function writeLastAppRoutePath(path: string | null): void {
  ...
  try {
    ...
    mollyStorage.setItem(LAST_APP_ROUTE_STORAGE_KEY, JSON.stringify(stored));
  } catch {
    // ignore
  }
}
