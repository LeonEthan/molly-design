...
export function createLocalStorageCache<T>(key: string, schema: z.ZodType<T>) {
  ...
  function readAll(): Record<string, T> {
    ...
    try {
      const raw = mollyStorage.getItem(key);
      if (!raw) return {};
  ...
  function set(id: string, value: T): void {
    ...
    try {
      const all = readAll();
      all[id] = value;
      mollyStorage.setItem(key, JSON.stringify(all));
    } catch {
      // ignore
    }
  }
...
export const PATH_LAUNCHER_PREFERENCE_STORAGE_KEY = 'molly:idePreference';
