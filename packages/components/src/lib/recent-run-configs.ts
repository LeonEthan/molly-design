...
export const RECENT_RUN_CONFIGS_KEY = 'molly:recentRunConfigs';
...
export function readRecentRunConfigs(
  ...
  try {
    const raw = mollyStorage.getItem(getRecentRunConfigsStorageKey(workspaceId));
    if (!raw) return [];
...
export function writeRecentRunConfigs(
  ...
  if (typeof window === 'undefined') return;
  try {
    mollyStorage.setItem(getRecentRunConfigsStorageKey(workspaceId), JSON.stringify(records));
  } catch {
    // ignore
  }
}
