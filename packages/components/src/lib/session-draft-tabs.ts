...
import { isValidSessionImagePathSegment, SessionIdSchema } from '@molly/shared';
import { z } from 'zod';
...
export const readPersistedDraftTabs = (parentSessionId: SessionId): DraftSessionTab[] => {
  ...
  return parseStoredDraftTabs(mollyStorage.getItem(getDraftTabsStorageKey(parentSessionId)));
};
...
export const readStoredTabOrder = (parentSessionId: SessionId): string[] => {
  ...
  try {
    const raw = mollyStorage.getItem(getTabOrderStorageKey(parentSessionId));
    if (!raw) {
      return [];
    }
...
export const writeStoredTabOrder = (parentSessionId: SessionId, tabOrder: string[]): void => {
  ...
  try {
    mollyStorage.setItem(getTabOrderStorageKey(parentSessionId), JSON.stringify(tabOrder));
  } catch {
    // ignore
  }
};
...
export const readStoredLastActiveTabState = (
  ...
  try {
    const raw = mollyStorage.getItem(getLastActiveTabStorageKey(parentSessionId));
    if (!raw) {
      return null;
    }
...
export const writeStoredLastActiveTabState = (
  ...
  try {
    mollyStorage.setItem(getLastActiveTabStorageKey(parentSessionId), JSON.stringify(state));
  } catch {
    // ignore
  }
};
