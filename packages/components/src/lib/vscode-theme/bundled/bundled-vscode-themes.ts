...
import mollyClassicLightJson from './themes/molly/molly-classic-light.json?raw';
import mollyDarkJson from './themes/molly/molly-dark.json?raw';
import mollyLightJson from './themes/molly/molly-light.json?raw';
import {
...
import type { MollyResolvedVSCodeTheme } from '../vscode-theme-schemas';

// The app ships exactly two palettes: Molly Light and Molly Dark (monochrome
// chrome, forked from Molly Light and Vesper) and the fixed selection below
// applies exactly those. Molly Light and Vesper remain selectable for
// reference/lineage; the rest of the bundled JSONs are not user-selectable.
const SELECTABLE_BUNDLED_VSCODE_THEME_IDS = new Set(['molly-classic-light', 'vesper', 'molly-light', 'molly-dark']);
...
const BUNDLED_THEME_EXTENSIONS: BundledVSCodeThemeExtension[] = [
  {
    // Molly's own light theme — forked from Vitesse Light Soft and tuned so the
    // derived app chrome (muted text, panels, composer field, borders) has
    // enough contrast. The syntax palette stays Vitesse.
    extensionId: 'molly.theme-classic',
    extensionVersion: '1.0.0',
    manifest: {
      name: 'theme-classic',
      displayName: 'Molly Theme',
      publisher: 'molly',
      version: '1.0.0',
      contributes: {
        themes: [
          {
            label: 'Molly Classic Light',
            uiTheme: 'vs',
            path: './themes/molly-classic-light.json',
          },
        ],
      },
    },
    files: {
      'themes/molly-classic-light.json': mollyClassicLightJson,
    },
    themeIdsByLabel: {
      'Molly Classic Light': 'molly-classic-light',
    },
  },
  {
    // Molly's monochrome chrome themes — Figma-language black/white
    // palette per output/visual-refresh-demo/visual-refresh-spec.zh.md.
    // molly-light is forked from molly-classic-light (Vitesse Light Soft lineage);
    // molly-dark is forked from Vesper. Syntax palettes stay as forked.
    extensionId: 'molly.theme-molly',
    extensionVersion: '1.0.0',
    manifest: {
      name: 'theme-molly',
      displayName: 'Molly Theme',
        ...
        themes: [
          {
            label: 'Molly Light',
            uiTheme: 'vs',
            path: './themes/molly-light.json',
          },
          {
            label: 'Molly Dark',
            uiTheme: 'vs-dark',
            path: './themes/molly-dark.json',
          },
        ],
      },
    },
    files: {
      'themes/molly-light.json': mollyLightJson,
      'themes/molly-dark.json': mollyDarkJson,
    },
    themeIdsByLabel: {
      'Molly Light': 'molly-light',
      'Molly Dark': 'molly-dark',
    },
  },
  {
...
let bundledThemeDescriptorsCache:
  ...
  | undefined;
let bundledThemesCache: MollyResolvedVSCodeTheme[] | undefined;
const bundledThemesById = new Map<string, MollyResolvedVSCodeTheme>();
...
export const getBundledVSCodeThemeById = async (
  themeId: string
): Promise<MollyResolvedVSCodeTheme | undefined> => getBundledVSCodeThemeByIdSync(themeId);

export const getBundledVSCodeThemeByIdSync = (
  themeId: string
): MollyResolvedVSCodeTheme | undefined => {
  if (themeId === 'lody-light') themeId = 'molly-classic-light';
  const cached = bundledThemesById.get(themeId === 'lody-light' ? 'molly-classic-light' : themeId);
  if (cached) {
    return cached;
  }
...
export const getCachedBundledVSCodeThemes = (): readonly MollyResolvedVSCodeTheme[] | undefined =>
  bundledThemesCache;

export const getCachedBundledVSCodeThemeById = (
  themeId: string
): MollyResolvedVSCodeTheme | undefined => bundledThemesById.get(themeId === 'lody-light' ? 'molly-classic-light' : themeId);

export const resolveBundledVSCodeThemes = async (): Promise<MollyResolvedVSCodeTheme[]> =>
  resolveBundledVSCodeThemesSync();

export const resolveBundledVSCodeThemesSync = (): MollyResolvedVSCodeTheme[] => {
