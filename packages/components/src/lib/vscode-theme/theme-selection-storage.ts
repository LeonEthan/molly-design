...
const fromThemeSelectionStorage = (
  ...
  const selection = compactThemeSelection({
    lightThemeId: storage.lightThemeId === 'lody-light' ? 'molly-classic-light' : storage.lightThemeId,
    darkThemeId: storage.darkThemeId,
  });

  // Migrate users who previously had "Molly default colors" (undefined theme IDs).
