...
type MollyAliasRule = {
  ...
  fallbackColorByThemeType?: Partial<Record<MollyResolvedVSCodeTheme['type'], string>>;
...
// Keep Molly's code block border theme-owned by resolving it only from VS Code border tokens.
...
const MOLLY_ALIAS_RULES: MollyAliasRule[] = [
...
export const createVSCodeThemeCssVariables = (
  theme: MollyResolvedVSCodeTheme
...
export const createMollyThemeCssVariables = (
  theme: MollyResolvedVSCodeTheme
): Record<string, string> => {
  const variables: Record<string, string> = {};
  const colorByCssVariable: Record<string, string> = {};

  for (const rule of MOLLY_ALIAS_RULES) {
 ...
 * VS Code themes are free to recess `input.background` (Molly Light does:
...
const createVisibleFallbackColor = (
  baseColor: string,
  themeType: MollyResolvedVSCodeTheme['type'],
  blendRatio: number
): string => {
...
export const createThemeCssVariables = (
  ...
  ...createMollyThemeCssVariables(theme),
});
