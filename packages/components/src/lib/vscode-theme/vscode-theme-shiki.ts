...
import type { MollyResolvedVSCodeTheme, TextMateTokenColorRule } from './vscode-theme-schemas';
...
export const createMollyVSCodeShikiThemeName = (theme: MollyResolvedVSCodeTheme): string =>
  `lody-vscode-${theme.id}`;

export const toShikiTheme = (
  theme: MollyResolvedVSCodeTheme,
  name: string = createMollyVSCodeShikiThemeName(theme)
...
export const getRegisteredMollyVSCodeThemeForDiffs = (
  theme: MollyResolvedVSCodeTheme,
  name: string = createMollyVSCodeShikiThemeName(theme)
): string | undefined => (registeredThemeNames.has(name) ? name : undefined);

export const registerMollyVSCodeThemeForDiffs = (
  theme: MollyResolvedVSCodeTheme,
  name: string = createMollyVSCodeShikiThemeName(theme)
): Promise<string | undefined> => {
  const registeredName = getRegisteredMollyVSCodeThemeForDiffs(theme, name);
  if (registeredName) {
