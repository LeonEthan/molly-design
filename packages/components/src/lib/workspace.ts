...
import { isReservedWorkspaceSlug, type Organization } from '@molly/shared';

const slugPattern = /^[a-z0-9]+(?:-[a-z0-9]+)*$/;
const maxSlugLength = 40;
const minSlugLength = 3;
const allowedSlugCharsPattern = /^[a-z0-9-]+$/;
const preferredWorkspaceSlugStorageKey = 'molly:preferredWorkspaceSlug';
...
export const readPreferredWorkspaceSlug = (): string | null => {
  ...
  try {
    const value = mollyStorage.getItem(preferredWorkspaceSlugStorageKey);
    if (!value) {
      return null;
    }
...
export const writePreferredWorkspaceSlug = (workspaceSlug: string | null): void => {
  ...
  try {
    if (!workspaceSlug) {
      mollyStorage.removeItem(preferredWorkspaceSlugStorageKey);
      return;
    }
    mollyStorage.setItem(preferredWorkspaceSlugStorageKey, workspaceSlug);
  } catch {
    // ignore
  }
};
