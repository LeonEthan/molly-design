...
import {
  MOLLY_MACHINE_MONITOR_OBSERVER_RENEW_MS,
  MOLLY_MACHINE_MONITOR_OBSERVER_TTL_MS,
  MOLLY_MACHINE_MONITOR_STATE_TTL_MS,
...
export class WorkspaceLocalMachineMonitorTransport {
  ...
  private readonly outboundStore = new EphemeralStore(MOLLY_MACHINE_MONITOR_STATE_TTL_MS);
  private readonly snapshotStore = new EphemeralStore(MOLLY_MACHINE_MONITOR_STATE_TTL_MS);
  ...
  private publishObserver(machineId: MachineId, forceSampleAtMs: number | null): void {
    ...
    const state: MachineMonitorObserverState = {
      ...
      updatedAtMs: nowMs,
      expiresAtMs: nowMs + MOLLY_MACHINE_MONITOR_OBSERVER_TTL_MS,
