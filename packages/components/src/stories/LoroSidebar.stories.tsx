...
function StoryLayout(args: Parameters<typeof LoroSidebar>[0]) {
  ...
  const allSidebarItems = useMemo<SidebarUpdatedItem[]>(
    () => buildDemoUpdatedItems(sessions),
    [sessions]
  );
...
function buildDemoUpdatedItems(source: SessionListRow[]): SidebarUpdatedItem[] {
...
const demoLocalSessions: SessionMeta[] = [
  ...
  // Independent Sessions the first one opened through the `molly_session_create`
