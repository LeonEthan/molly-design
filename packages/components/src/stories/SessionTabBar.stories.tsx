...
function buildPresenceStates(presence: StoryPresence) {
  return Object.fromEntries(
    Object.entries(presence).map(([sessionId, type]) => {
      const instanceId = `storybook-${sessionId}` as MollyPresenceInstanceId;
      return [
        getMollySessionPresenceKey(sessionId as SessionId, instanceId),
        {
...
function StoryShell({
  ...
  const [store] = useState(() => {
    ...
    nextStore.set(mollyPresenceStatesAtom, buildPresenceStates(presence));
    return nextStore;
  });
