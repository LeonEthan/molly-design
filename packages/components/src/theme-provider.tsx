...
import {
  ...
  createMollyVSCodeShikiThemeName,
  getBundledVSCodeThemeByIdSync,
  getCachedBundledVSCodeThemes,
  getRegisteredMollyVSCodeThemeForDiffs,
  isSelectableBundledVSCodeThemeId,
  resolveBundledVSCodeThemes,
  registerMollyVSCodeThemeForDiffs,
  type MollyResolvedVSCodeTheme,
 ...
 * chrome, forked from Molly Light and Vesper). Users pick light, dark, or
 * system (follow the OS). The underlying VS Code theme selection is FIXED —
 * there is no per-mode theme picker or persistence. The bundled VS Code theme
 * machinery stays only to drive syntax colors for the editor/diff/terminal at
 * these two ids.
 */
...
function MollyThemeProvider({
  children,
  defaultTheme,
  previewedTheme,
  setPreviewedTheme,
}: MollyThemeProviderProps) {
  const {
...
export const useActiveVSCodeTheme = (): MollyResolvedVSCodeTheme | undefined => {
...
export const useBundledVSCodeThemes = (): MollyResolvedVSCodeTheme[] => {
  const [loaded, setLoaded] = useState(() => getCachedBundledVSCodeThemes() !== undefined);
  const [themes, setThemes] = useState<MollyResolvedVSCodeTheme[]>(() => [
    ...(getCachedBundledVSCodeThemes() ?? []),
  ]);
...
export const useSelectableBundledVSCodeThemes = (): MollyResolvedVSCodeTheme[] => {
...
export const useActiveVSCodeDiffThemeName = (): string | undefined => {
  ...
  const activeDiffThemeName = activeTheme ? createMollyVSCodeShikiThemeName(activeTheme) : undefined;
  const [registeredDiffThemeName, setRegisteredDiffThemeName] = useState<string | undefined>(() =>
    activeTheme ? getRegisteredMollyVSCodeThemeForDiffs(activeTheme) : undefined
  );
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    const registeredName = getRegisteredMollyVSCodeThemeForDiffs(activeTheme);
    ...
    let cancelled = false;
    void registerMollyVSCodeThemeForDiffs(activeTheme).then((name) => {
      if (!cancelled) {
  ...
  const cachedRegisteredName = activeTheme
    ? getRegisteredMollyVSCodeThemeForDiffs(activeTheme)
    : undefined;
  return (
