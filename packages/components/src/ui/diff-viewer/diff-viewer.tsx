...
function DiffViewerImpl({
  ...
  const prerenderCacheKey = useMemo(() => {
    ...
    return `${parseCacheKey}:${effectiveOldTextLength}:${effectiveNewTextLength}:${effectiveDiffStyle}:${defaultLineDiffType}:${resolvedTheme}:${activeDiffThemeName ?? 'molly-default'}`;
  }, [
    activeDiffThemeName,
    cachePrerenderedHtml,
    defaultLineDiffType,
  ...
  const diffContentContainer = (
    <div
      ref={diffHostRef}
      className="molly-pierre-diff w-full bg-background"
