...
function DraftHarness({ draftKey }: { draftKey: string }) {
  ...
  harness = {
    ...
    // What submit-accepted and an explicit discard call in `chat-landing.tsx`.
    clearDraft: () => {
...
describe('chat landing draft persistence', () => {
  ...
  it('uses the same durable workspace draft across independent remounts', () => {
    mountLanding(WORKSPACE_A_KEY);
    act(() => {
  ...
  it('clears the draft for good once submission acceptance releases it', async () => {
    mountLanding(WORKSPACE_A_KEY);
    act(() => {
      readHarness().addImages([pngFile('shot.png')]);
    });
