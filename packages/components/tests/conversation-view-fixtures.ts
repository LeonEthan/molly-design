import { sessionDocSchema, type SessionHistory, type SessionId } from '@molly/shared';
...
export async function openReaderView(
  ...
  const { createLoroSessionData } = await import('@molly/shared/session-data');
...
export function writeStoredField(
  data: import('@molly/shared/session-data').LoroSessionData,
  id: string,
  key: import('@molly/shared/session-data').SessionWritableField,
