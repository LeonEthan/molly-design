...
import type { SessionHistory } from '@molly/shared';
...
const flush = flushReaderChanges;
...
describe.each(backends)('createConversationViewFromReader over $name', (backend) => {
  it('updates off-window design receipts and provider identity without hydrating that body', async () => {
    const harness = openView(backend, 30, { tailKeep: 2 });
    const { view, data } = harness;
    try {
      await waitTurns(view, harness.expected.length);
      const row = view.index(0)!;
      ...
      await data.commands.applyHistoryAction({ kind: 'design-outcome', turnId: row.id, outcome });
      data.writer.updateEntry(row.id, (entry) => ({
        ...entry,
        inputConfig: { ...entry.inputConfig, agentConfigId: 'provider-exact' },
      }));
      await flushReaderChanges();
      expect(view.index(0)?.designOutcome).toEqual(outcome);
      expect(view.index(0)?.inputConfig?.agentConfigId).toBe('provider-exact');
      expect(view.isHydrated(0)).toBe(false);
    } finally {
      view.dispose();
      harness.teardown();
  ...
  it.each([50, 500])('rereads only the changed tail with %i rounds', async (rounds) => {
    const harness = openView(backend, rounds, { tailKeep: 4 });
    const { idle, data } = harness;
    const probe = probeReader(data.history);
    const probeView = createConversationViewFromReader(probe.reader, {
      sessionId: FIXTURE_SESSION_ID,
  ...
  it.each([50, 500])(
    'rereads only the named early and tail turns with %i rounds',
    async (rounds) => {
  ...
  it.each([50, 500])('refreshes off-window send configuration with %i rounds', async (rounds) => {
    const harness = openView(backend, rounds, { tailKeep: 2, maxHydrated: 4 });
    const { idle, data } = harness;
    try {
...
describe('createConversationViewFromReader Loro-only wiring', () => {
  it('windowed createConversationSession builds its history from the session-data port', async () => {
    ...
    try {
      await settle(idle, session.history);
