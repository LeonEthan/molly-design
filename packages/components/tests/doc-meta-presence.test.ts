...
import {
  mollyPresenceStatesAtom,
  mollyPresenceSyncStateAtom,
...
describe('doc meta presence overlay', () => {
  it('keeps session metadata durable while exposing fresh live session status', () => {
    ...
    store.set(mollyPresenceStatesAtom, {
      ...
      [getMollySessionPresenceKey(childSessionId, instanceId)]: {
        kind: 'session',
    ...
    store.set(mollyPresenceSyncStateAtom, 'synced');
    ...
    store.set(mollyPresenceSyncStateAtom, 'disconnected');
    expect(store.get(machineOnlineStatusAtomFamily('missing-machine' as MachineId))).toBe(
      'unknown'
    );
  ...
  it('preserves session references when only sibling presence changes', () => {
    ...
    const instanceId = 'stable-instance-1' as MollyPresenceInstanceId;
    const siblingInstanceId = 'stable-instance-2' as MollyPresenceInstanceId;
    const sessionRoomId = getSessionRoomId(sessionId);
    const childSessionRoomId = getSessionRoomId(childSessionId);
    ...
    store.set(mollyPresenceStatesAtom, {
      ...
      [getMollySessionPresenceKey(siblingSessionId, siblingInstanceId)]: {
        kind: 'session',
  ...
  it('expires live session status when the presence clock advances past TTL', () => {
    ...
    const instanceId = 'ttl-instance-1' as MollyPresenceInstanceId;
    ...
    store.set(setMollyPresenceNowMsAtom, now + MOLLY_PRESENCE_TTL_MS + 1);
