...
import { applyTextRewrites, reanchorMessageTextSpansForTrim } from '@molly/shared';
...
describe('before-send mention rewrite', () => {
  ...
  it('keeps every span addressing the output when rewrites of different lengths mix', () => {
    ...
    expect(result.text).toContain('use molly mcp to query session[id: sess-9f2c] history');
