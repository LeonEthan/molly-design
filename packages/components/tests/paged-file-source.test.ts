...
import { LOCAL_TEXT_PAGE_BYTES } from '@molly/shared/local-file-preview';
...
describe('local paged text', () => {
  it.each(['中', '😀', 'é'])(
    ...
    async (character) => {
      for (let overlap = 1; overlap < new TextEncoder().encode(character).length; overlap++) {
        ...
        const source = createLocalPagedFileSource('molly-resource://file/test', bytes.length);
  ...
  it('refuses stale resources and invalid pages', async () => {
    ...
    const source = createLocalPagedFileSource('molly-resource://file/test', 100);
