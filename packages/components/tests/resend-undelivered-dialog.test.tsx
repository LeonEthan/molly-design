...
describe('UserMessageRowView undelivered resend dialog', () => {
  ...
  it('keeps unknown delivery non-dispatchable and does not expose resend recovery', async () => {
    ...
    await renderRow(
      ...
      { ...undeliveredMessage, status: 'delivery_unknown', read: false }
    );
    expect(document.body.textContent).toContain('Application unknown');
    expect(document.body.textContent).not.toContain('Not delivered');
    expect(
      [...document.querySelectorAll('button')].some((button) =>
        button.textContent?.includes('Application unknown')
      )
    ).toBe(false);
    expect(document.body.textContent).not.toContain('Resend message');
