...
import { getMachineRoomId } from '@molly/shared';
...
describe('SessionBrowserPanel controller', () => {
  ...
  it('rejects a remote host preview before creating a tunnel', async () => {
    ...
    expect(rendered.textContent).toContain('Remote host previews are unavailable.');
    expect(testRuntime.requestSessionPreviewCreate).not.toHaveBeenCalled();
    expect(testRuntime.requestSessionPreviewEndpointAcquire).not.toHaveBeenCalled();
  });
  ...
  it('keeps a composer bar request pending until a reported candidate reaches the session doc', async () => {
    ...
    window.__MOLLY_ELECTRON__ = true;
    const testRuntime = createRuntime({ plane: 'local', sessionStore: catchingUp.store });
    const rendered = await renderPanel(testRuntime.runtime, {
    ...
    expect(testRuntime.requestSessionPreviewEndpointAcquire).toHaveBeenCalledOnce();
