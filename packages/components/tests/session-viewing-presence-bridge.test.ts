...
describe('session-viewing presence over EphemeralStore', () => {
  it('propagates set/delete between bridged stores and survives schema parsing', () => {
    ...
    const key = getMollySessionViewingPresenceKey('user-1', 'instance-1' as MollyPresenceInstanceId);
    const state = {
    ...
    expect(
      parseMollyPresenceStates(observer.getAllStates() as Record<string, unknown>)[key]
    ).toEqual(state);
