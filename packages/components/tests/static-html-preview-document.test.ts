...
import { STATIC_HTML_PREVIEW_DOCUMENT_MARKER } from '@molly/shared';
...
describe('static HTML preview document', () => {
  it('installs the owned policy and annotation runtime before source scripts', () => {
    ...
    expect(headElements[3]?.getAttribute('data-molly-visual-annotation-runtime')).toBe('true');
    ...
    expect(document.querySelector('script[data-molly-visual-annotation-runtime="true"]')).not.toBe(
      null
    );
  });
