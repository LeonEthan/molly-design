...
const FIXED_LIGHT_THEME_ID = 'molly-light';
const FIXED_DARK_THEME_ID = 'molly-dark';
...
describe('ThemeProvider', () => {
  ...
  afterEach(() => {
    ...
    delete document.documentElement.dataset.mollyVscodeTheme;
  ...
  it('replaces previously applied theme variables even if the old application is not explicitly disposed', () => {
    ...
    expect(documentRoot.dataset.mollyVscodeTheme).toBe('vscode-light-2026');
    ...
    expect(documentRoot.dataset.mollyVscodeTheme).toBeUndefined();
    expect(documentRoot.style.getPropertyValue('--vscode-button-background')).toBe('');
  });
  ...
  it('pins the fixed VS Code theme per mode and persists the explicit mode', () => {
    ...
    expect(document.documentElement.dataset.mollyVscodeTheme).toBe(FIXED_LIGHT_THEME_ID);
