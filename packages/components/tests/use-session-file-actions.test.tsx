...
describe('local-host actions vs a resolvable host path', () => {
  ...
  beforeEach(async () => {
    ...
    localHomeDir = '/home/dev';
    (window as { __MOLLY_ELECTRON__?: boolean }).__MOLLY_ELECTRON__ = true;
    (window as { __MOLLY_PLATFORM__?: { os: string } }).__MOLLY_PLATFORM__ = { os: 'darwin' };
    container = document.createElement('div');
    document.body.appendChild(container);
    root = createRoot(container);
  });
  ...
  afterEach(async () => {
    ...
    container?.remove();
    container = undefined;
    delete (window as { __MOLLY_ELECTRON__?: boolean }).__MOLLY_ELECTRON__;
    delete (window as { __MOLLY_PLATFORM__?: { os: string } }).__MOLLY_PLATFORM__;
    localMachineId = null;
    localHomeDir = '/home/dev';
    vi.restoreAllMocks();
  });
  ...
  it('passes a local parent-relative DMG to Finder and the default application', async () => {
    const actions = await resolveActions();
    ...
    await act(async () => {
      ...
      actions.localHost?.openInDefaultApp(filePath);
    });
    expect(revealLocalPath).toHaveBeenLastCalledWith(hostPath);
    expect(openLocalPath).toHaveBeenLastCalledWith(hostPath);
    expect(toastError).not.toHaveBeenCalled();
  });
