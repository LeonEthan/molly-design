...
const createTheme = (id: string): MollyResolvedVSCodeTheme => ({
  schemaVersion: 1,
  id,
  label: `Theme ${id}`,
  type: 'dark',
...
describe('VSCode diff theme registration', () => {
  ...
  it('does not report a lazy custom theme as registered until @pierre/diffs has the loader', async () => {
    ...
    const themeName = createMollyVSCodeShikiThemeName(theme);

    const firstRegistration = registerMollyVSCodeThemeForDiffs(theme);
    const secondRegistration = registerMollyVSCodeThemeForDiffs(theme);

    expect(firstRegistration).toBe(secondRegistration);
    expect(getRegisteredMollyVSCodeThemeForDiffs(theme)).toBeUndefined();
    ...
    expect(getRegisteredMollyVSCodeThemeForDiffs(theme)).toBe(themeName);
  ...
  it('keeps the React hook on fallback until the lazy custom theme registration finishes', async () => {
    ...
    try {
      ...
      for (let attempts = 0; attempts < 10; attempts += 1) {
        ...
        if (probe?.dataset.diffThemeName === 'lody-vscode-molly-dark') {
          break;
        }
      }

      expect(probe?.dataset.diffThemeName).toBe('lody-vscode-molly-dark');
    } finally {
      flushSync(() => {
        root?.unmount();
      });
