...
import { createMollyThemeCssVariables } from '../src/lib/vscode-theme/vscode-theme-css';
import {
  compositeHexColors,
  hexColorToHslChannel,
} from '../src/lib/vscode-theme/vscode-theme-color';
import type { MollyResolvedVSCodeTheme } from '../src/lib/vscode-theme/vscode-theme-schemas';

const themeFixture: MollyResolvedVSCodeTheme = {
  schemaVersion: 1,
  id: 'vesper-like',
  label: 'Vesper-like',
  type: 'dark',
...
describe('createLodyThemeCssVariables', () => {
  it('maps sidebar and interaction aliases from VSCode workbench colors', () => {
    const variables = createMollyThemeCssVariables(themeFixture);
  ...
  it('falls back to neutral workbench surfaces when the theme does not define secondary button tokens', () => {
    ...
    const variables = createMollyThemeCssVariables({
      ...themeFixture,
