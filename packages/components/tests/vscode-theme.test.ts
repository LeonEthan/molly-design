...
describe('VSCode theme adapter', () => {
  ...
  it('creates raw VSCode variables, Molly HSL aliases, and Prism-compatible syntax aliases', async () => {
    const theme = await resolveVSCodeTheme({
      id: 'fixture-dark',
      label: 'Fixture Dark',
      uiTheme: 'vs-dark',
    ...
    expect(createMollyThemeCssVariables(theme)).toMatchObject({
  ...
  it('exposes only the product theme shortlist in selectors', async () => {
    ...
    expect(
      themes
        .filter((theme) => isSelectableBundledVSCodeThemeId(theme.id))
        .map((theme) => theme.id)
        .sort()
    ).toEqual(['molly-classic-light', 'molly-dark', 'molly-light', 'vesper']);
  });

  it('gives Molly Light legible muted text and recessed panels', async () => {
    ...
    const light = await getBundledVSCodeThemeById('molly-classic-light');
    if (!light) throw new Error('Expected molly-classic-light to be bundled');
    const vars = createMollyThemeCssVariables(light);
    ...
    // floor), so a regression in molly-classic-light.json is caught here.
  ...
  it('never draws a form field darker than the page it sits on', async () => {
    ...
    for (const theme of await resolveBundledVSCodeThemes()) {
      const vars = createMollyThemeCssVariables(theme);
  ...
  it('keeps selectable sidebars visibly offset from the main background', async () => {
    ...
    for (const theme of themes) {
      ...
      /* The molly-dark sidebar border is an intentional hairline per the frozen
         monochrome spec (§2: --border #383838 on panel #2c2c2c ≈ 1.19) — the
         dark panel offset is carried by the canvas-ground gap, not the border
         line, so the floor stays just below that pair. */
      expect(borderContrast, theme.id).toBeGreaterThanOrEqual(1.15);
      expect(variables['--sidebar-border'], theme.id).toBe(
        hexColorToHslChannel(sidebarBorderColor)
      );

      // The offset DIRECTION is a per-theme design choice, asserted explicitly
      // so a regression cannot silently flip an approved hierarchy. Classic
      // light keeps the sidebar recessed (darker than the editor); every dark
      // theme — and the Molly monochrome light palette, whose canvas ground
      // (#f5f5f5) deliberately sits lighter than the white panel — offsets
      // brighter (frozen spec §2).
      const offsetDirectionByTheme: Record<string, 'darker' | 'brighter'> = {
        'molly-classic-light': 'darker',
        vesper: 'brighter',
        'molly-light': 'brighter',
        'molly-dark': 'brighter',
      };
      const direction = offsetDirectionByTheme[theme.id];
      if (direction === undefined) continue;
      const editorLuminance = relativeLuminance(theme.colors['editor.background'] ?? '');
      if (direction === 'darker') {
        expect(sidebarLuminance, theme.id).toBeLessThan(editorLuminance);
      } else {
        expect(sidebarLuminance, theme.id).toBeGreaterThan(editorLuminance);
      }
    }
  });
