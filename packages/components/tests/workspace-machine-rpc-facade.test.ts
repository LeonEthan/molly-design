...
describe('createWorkspaceMachineRpcFacade', () => {
  ...
  it('rejects a remote machine without attempting local IPC', async () => {
    const invoke = vi.fn();
    vi.stubGlobal('window', { __MOLLY_ELECTRON__: true, ipc: { invoke } });
    const facade = createWorkspaceMachineRpcFacade({
      workspaceId,
      getMachineProtocolCapabilities: async () => CURRENT_MACHINE_PROTOCOL_CAPABILITIES,
      targetRouter: {
    ...
    await expect(
      ...
      type: 'session/cancel_response',
      success: false,
      error: expect.stringContaining('Local Machine RPC is unavailable'),
    });
    expect(invoke).not.toHaveBeenCalled();
  });
});
