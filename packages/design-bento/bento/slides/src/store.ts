...
export class Store {
  ...
  /** A1a-2 adapter only: refresh the rendered view without creating Store history. */
  replaceViewDoc(next: BentoDoc) {
    const before = this.captureView()
    this.doc = next
    const { currentChanged, selectionChanged } = this.reconcileView(before)
    this.setDirty(true)
    this.emit('slides')
