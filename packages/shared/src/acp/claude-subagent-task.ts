 ...
 * `subagent_task` item.
 */
export const MOLLY_CLAUDE_TASK_LIFECYCLE_RAW_INPUT_KEY = 'lodyClaudeTaskLifecycle';
/** Provider-neutral carrier used by newer builtin ACP adapters. */
export const MOLLY_SUBAGENT_TASK_LIFECYCLE_RAW_INPUT_KEY = 'lodySubagentTaskLifecycle';
...
const MollyTaskMetaSchema = z.object({
  version: z.literal(1),
  taskId: z.string().min(1),
...
export const parseMollyTaskMeta = (meta: unknown): SubagentTaskPayload | null => {
  if (!isRecord(meta) || !isRecord(meta.lody)) return null;
  const parsed = MollyTaskMetaSchema.safeParse(meta.lody.task);
...
export const parseSubagentTaskWire = (rawInput: unknown): SubagentTaskPayload | null => {
  ...
  const carrier =
    rawInput[MOLLY_SUBAGENT_TASK_LIFECYCLE_RAW_INPUT_KEY] ??
    rawInput[MOLLY_CLAUDE_TASK_LIFECYCLE_RAW_INPUT_KEY];
