...
import { sanitizeMollyInternalInstructions } from '../goal';
  ...
  for (const item of items) {
    const nextItem =
      item.type === 'text' || item.type === 'thought'
        ? ({ ...item, text: sanitizeMollyInternalInstructions(item.text) } as MessageContent)
        : item;
    ...
    if (
      ...
      const text = sanitizeMollyInternalInstructions(existing.text + next.text);
      if (text) {
...
const parseCodexProposedPlanTags = (text: string, turnId: string): MessageContent[] => {
  // Codex may still emit proposed-plan markup as ordinary assistant text. Keep parsing in Molly,
...
export const buildMessageContentFromNotification = (
  ...
  switch (update.sessionUpdate) {
    ...
    case 'tool_call_update': {
      ...
      const subagentTask =
        parseMollyTaskMeta((update as ToolCallUpdateWithMeta)._meta) ??
...
class NotificationOnHistoryApplier {
  ...
  private applyMessageContent(message: MessageContent) {
    switch (message.type) {
      case 'text': {
        const text = sanitizeMollyInternalInstructions(message.text);
        if (!text) return;
        const entryIndex = this.ensureActiveAssistantEntry();
  ...
  private appendOrMergeAdjacentText(
    ...
    if (last && last.type === kind) {
      ...
      const text = sanitizeMollyInternalInstructions(mergeStreamChunk(existing.text, delta));
      if (!text) {
        items.pop();
