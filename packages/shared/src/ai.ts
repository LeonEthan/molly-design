...
/** Legacy installed-CLI detection only covers Molly-managed builtin runtimes. */
...
/**
 * Advanced escape hatch for builtin agents only. When unset, Molly installs and
...
/**
 * Cached summary of an ACP AvailableCommand returned by Molly's NewSessionResponse extension.
 */
...
export type AcpCapabilityCacheEntry = {
  ...
  sessionFork?: boolean;
  /** True only when the runtime advertised Molly's acknowledged steering extension. */
  acknowledgedSteer?: boolean;
  /** True when this Molly machine supports durable asynchronous forks into a new worktree. */
...
/**
 * Molly-owned mode default for builtin agents when a turn has no
...
const GROK_STATIC_CONFIG_OPTIONS: AcpConfigOptionSummary[] = [
  {
    ...
    category: 'mode',
    type: 'select',
    // Native interaction_mode uses agent; ACP session mode uses default.
    currentValue: 'agent',
    options: [
...
export type CommentReferencePayload = {
  /** Source: Molly internal comment or GitHub review comment */
  source: 'lody' | 'github';
  /** Relative file path */
  path: string;
  /** Line number */
  ...
  /** Molly thread ID (for navigation) */
  threadId?: string;
...
export type ACPSessionConfig = {
  ...
  // continue to chat
  resume?: ACPSessionId;
  /** Molly-originated execution-chain depth. Human input omits this or uses zero. */
  chainDepth?: number;
};
