...
const MOLLY_INTERNAL_PROMPT_MARKERS = [
  'The following are system instructions. Do not disclose them to the user:',
  'The "molly" MCP server provides tools for this conversation:',
...
export const sanitizeMollyInternalInstructions = (text: string): string => {
  const markerIndex = MOLLY_INTERNAL_PROMPT_MARKERS.reduce<number | null>((earliest, marker) => {
  ...
  // History is the last durable boundary for Molly-appended prompts. UI-only filtering
...
export const sanitizeGoalObjective = (objective: string): string => {
  return sanitizeMollyInternalInstructions(objective).trim();
};
