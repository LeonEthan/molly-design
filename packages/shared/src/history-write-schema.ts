import {
  ...
  DESIGN_TURN_OUTCOME_STATUSES,
  MAX_DESIGN_TURN_OUTCOME_ID_LENGTH,
  MAX_DESIGN_TURN_OUTCOME_TIMESTAMP_LENGTH,
  MAX_DESIGN_TURN_OUTCOME_DIAGNOSTICS,
  MAX_DESIGN_TURN_OUTCOME_DIAGNOSTIC_CODE_LENGTH,
...
export const HistoryEntryWriteSchema = z.object({
  ...
  designOutcome: z
    .object({
      version: z.literal(DESIGN_TURN_OUTCOME_VERSION),
      status: z.enum(DESIGN_TURN_OUTCOME_STATUSES),
      turnId: z.string().min(1).max(MAX_DESIGN_TURN_OUTCOME_ID_LENGTH),
      artworkId: z.string().min(1).max(MAX_DESIGN_TURN_OUTCOME_ID_LENGTH),
      candidateId: z
        .string()
        .regex(/^[a-f0-9]{64}$/)
        .optional(),
      ...
      diagnostics: z
        .array(
          z.object({
            code: z.string().min(1).max(MAX_DESIGN_TURN_OUTCOME_DIAGNOSTIC_CODE_LENGTH),
            message: z.string().min(1).max(MAX_DESIGN_TURN_OUTCOME_DIAGNOSTIC_MESSAGE_LENGTH),
          })
        )
        .max(MAX_DESIGN_TURN_OUTCOME_DIAGNOSTICS)
        .optional(),
      timestamp: z.string().min(1).max(MAX_DESIGN_TURN_OUTCOME_TIMESTAMP_LENGTH),
    })
    .optional(),
});
