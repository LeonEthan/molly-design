...
const { getMollyDataDir } = require('./installation-profile.cjs');

/**
 * Lock file directory:
 * - default: ~/.lody/locks/
 * - override: $MOLLY_LOCKS_DIR
 */
function resolveLocksDir(override) {
  ...
  const fromEnv = (process.env.MOLLY_LOCKS_DIR ?? process.env.LODY_LOCKS_DIR) ? String((process.env.MOLLY_LOCKS_DIR ?? process.env.LODY_LOCKS_DIR)).trim() : '';
  if (fromEnv) return path.resolve(fromEnv);

  return path.join(getMollyDataDir(), 'locks');
}
...
async function withInProcessQueue(key, fn) {
