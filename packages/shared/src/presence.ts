...
export const MOLLY_PRESENCE_CHANNEL = 'presence';
export const MOLLY_PRESENCE_TTL_MS = 90_000;
export const MOLLY_PRESENCE_HEARTBEAT_MS = 30_000;

export type MollyPresenceInstanceId = string & { __brand: 'LodyPresenceInstanceId' };

export type MollyMachinePresenceState = {
  kind: 'machine';
  machineId: MachineId;
  instanceId: MollyPresenceInstanceId;
  updatedAt: number;
};

export type MollySessionPresenceState = {
  kind: 'session';
  sessionId: SessionId;
  machineId: MachineId;
  instanceId: MollyPresenceInstanceId;
 ...
 */
export type MollySessionViewingPresenceState = {
...
export type MollyPresenceState =
  | MollyMachinePresenceState
  | MollySessionPresenceState
  | MollySessionViewingPresenceState;
export type MollyPresenceStateMap = Record<string, MollyPresenceState>;
...
const PresenceInstanceIdSchema = z
  ...
  .min(1)
  .transform((value) => {
    return value as MollyPresenceInstanceId;
  });
...
export const MollyPresenceStateSchema = z.discriminatedUnion('kind', [
  PresenceMachineStateSchema,
  PresenceSessionStateSchema,
  PresenceSessionViewingStateSchema,
]);

export const getMollyMachinePresenceKey = (
  machineId: MachineId,
  instanceId: MollyPresenceInstanceId
): string => `machine:${encodeURIComponent(machineId)}:${encodeURIComponent(instanceId)}`;

export const getMollySessionPresenceKey = (
  sessionId: SessionId,
  instanceId: MollyPresenceInstanceId
): string => `session:${encodeURIComponent(sessionId)}:${encodeURIComponent(instanceId)}`;

export const getMollySessionViewingPresenceKey = (
  userId: string,
  instanceId: MollyPresenceInstanceId
): string => `viewing:${encodeURIComponent(userId)}:${encodeURIComponent(instanceId)}`;

export const toMollyPresenceStreamUrl = (durableStreamUrl: string): string => {
  const url = new URL(durableStreamUrl);
  url.searchParams.set('ephemeral', MOLLY_PRESENCE_CHANNEL);
  return url.toString();
};

export const parseMollyPresenceStates = (states: Record<string, unknown>): MollyPresenceStateMap => {
  const parsed: MollyPresenceStateMap = {};
  for (const [key, value] of Object.entries(states)) {
    const result = MollyPresenceStateSchema.safeParse(value);
...
export const isFreshMollyPresenceState = (
  state: Pick<MollyPresenceState, 'updatedAt'>,
  nowMs: number,
  ttlMs: number = MOLLY_PRESENCE_TTL_MS
): boolean => {
  return Number.isFinite(state.updatedAt) && nowMs - state.updatedAt < ttlMs;
};

export const findFreshMachinePresenceState = (
  states: MollyPresenceStateMap,
  machineId: MachineId,
  nowMs: number,
  ttlMs: number = MOLLY_PRESENCE_TTL_MS
): MollyMachinePresenceState | undefined => {
  let latest: MollyMachinePresenceState | undefined;
  for (const state of Object.values(states)) {
    if (state.kind !== 'machine' || state.machineId !== machineId) continue;
    if (!isFreshMollyPresenceState(state, nowMs, ttlMs)) continue;
...
export const findFreshSessionPresenceState = (
  ...
  nowMs: number,
  ttlMs: number = MOLLY_PRESENCE_TTL_MS
): MollySessionPresenceState | undefined => {
  let latest: MollySessionPresenceState | undefined;
  for (const state of Object.values(states)) {
