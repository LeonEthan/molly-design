...
- Molly's `design-outcome` command preserves the first receipt. Shallow directory rows
  expose bounded, validated receipts and exact `agentConfigId` even outside the body window.
  Generic status projection cannot revive `delivery_unknown`; authoritative application
  evidence is an explicit command option, never inferred from a timeout or user retry.
