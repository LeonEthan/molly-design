...
import type { StoredMollyOperation } from '../session-orchestration';
...
export type HistoryAction =
  | {
      kind: 'design-outcome';
      turnId: string;
      outcome: import('../design-turn-outcome').DesignTurnOutcome;
    }
  | {
  ...
  | { kind: 'operation-completion'; operation: StoredMollyOperation; turn: SessionEntry }
  ...
  | {
      ...
      /** Exact native application evidence, never inferred from a timeout or absence. */
      authoritativeDelivery?: boolean;
    }
  | {
...
export function historyActionTarget(action: HistoryAction): string | undefined {
  return action.kind === 'design-outcome' ||
    action.kind === 'assistant-items' ||
...
export function applyHistoryAction(
  ...
  switch (action.kind) {
    case 'design-outcome': {
      const entry = history.find((turn) => turn.id === action.turnId);
      if (!entry || entry.designOutcome != null) return { turns: history, matched: false };
      entry.designOutcome = action.outcome;
    ...
    case 'user-status': {
      ...
      if (
        action.steerProjection &&
        !['pending_apply', 'processing'].includes(entry.status ?? '') &&
        !(action.authoritativeDelivery && entry.status === 'delivery_unknown')
      )
        return { turns: history, matched: false };
      if (action.requeueUndelivered) {
        if (!['pending_apply', 'pending', 'seen'].includes(entry.status ?? ''))
          return { turns: history, matched: false };
        if (entry.status !== 'pending_apply' && entry.status !== 'delivery_unknown')
          return { turns: history, matched: true };
      }
      entry.status = action.status;
      entry.read = !['pending', 'pending_apply', 'delivery_unknown'].includes(action.status);
