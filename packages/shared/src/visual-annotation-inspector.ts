...
const INTERACTION_LAYER_ATTRIBUTE = 'data-molly-visual-annotation-interaction-layer';
...
export const createVisualCommentInspector = (
  ...
  const warnOnce = (message: string, key: 'allowedParentOrigins' | 'postMessageTargetOrigin') => {
    ...
    globalThis.console.warn(`[Molly visual annotation] ${message}`);
  };
  ...
  const getOverlay = () => {
    ...
    const next = runtimeDocument.createElement('div');
    next.setAttribute('data-molly-visual-annotation-overlay', 'true');
...
const isInspectorOverlay = (element: Element): boolean =>
  element.closest(
    `[data-molly-visual-annotation-overlay="true"], [${INTERACTION_LAYER_ATTRIBUTE}="true"]`
  ) !== null;
