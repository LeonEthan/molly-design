...
/**
 * Base directory for all Molly repositories on the host.
 * Returns `<homeDir>/.lody/repos`, in the separator style of `homeDir` itself.
 *
 * Carries the same caveat as `getMollyDotlodyPath`: the literal `.lody` is a fallback
 * for a machine with no published `dotlodyPath` row, not the installation's real data
 * root. Node callers have `getMollyDataDir()` and must use it.
 *
 ...
 */
export function getMollyReposBaseDir(homeDir: string): string {
  return getMollyReposBaseDirFromDotlodyPath(getMollyDotlodyPath(homeDir));
}

/**
 * Host path for the machine-wide Molly data directory.
 ...
 */
export function getMollyDotlodyPath(homeDir: string): string {
...
/**
 * Base directory for all Molly repositories from a stored machine `.lody` path.
 * Returns `<dotlodyPath>/repos`, in the separator style of `dotlodyPath` itself.
 */
export function getMollyReposBaseDirFromDotlodyPath(dotlodyPath: string): string {
...
export function getWorktreeHostPath(
  ...
  assertSafeSessionId(sessionId);
  if (homeDir) {
    return getWorktreeHostPathFromDotlodyPath(repoId, sessionId, getMollyDotlodyPath(homeDir));
  }
