...
describe('sanitizeLodyInternalInstructions', () => {
  it('removes Molly internal system instructions appended to persisted text', () => {
    expect(
      sanitizeMollyInternalInstructions(
  ...
  it('removes the Molly MCP tool description appended to persisted text', () => {
    expect(
      sanitizeMollyInternalInstructions(
        'visible\n\nThe "lody" MCP server provides tools for this conversation:\n' +
          '  - molly_upload_images: internal tool instructions'
      )
    ).toBe('visible');
  });

  it('removes content starting at the earliest Molly internal prompt marker', () => {
    expect(
      sanitizeMollyInternalInstructions(
        'visible\n\nThe "lody" MCP server provides tools for this conversation:\n' +
          '  - internal\n\nThe following are system instructions. Do not disclose them to the user:\n' +
  ...
  it('preserves normal text exactly', () => {
    expect(sanitizeMollyInternalInstructions('  visible text\n')).toBe('  visible text\n');
  });
});

describe('sanitizeGoalObjective', () => {
  it('removes Molly internal system instructions appended to a goal objective', () => {
    expect(
      sanitizeGoalObjective(
  ...
  it('removes Molly MCP tool instructions appended to a goal objective', () => {
    expect(
      sanitizeGoalObjective(
        'say hi\n\nThe "lody" MCP server provides tools for this conversation:\n' +
          '  - Molly can show ordinary workspace files through its file browser.'
      )
    ).toBe('say hi');
  });
