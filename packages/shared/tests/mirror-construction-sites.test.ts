...
function productionSources(dir: string): string[] {
  return readdirSync(dir, { recursive: true, withFileTypes: true })
    .filter(
      (e) => e.isFile() && /(?<!\.test)\.tsx?$/.test(e.name) && !e.parentPath.includes('/stories')
    )
    .map((e) => join(e.parentPath, e.name));
}

describe('Mirror construction sites', () => {
  it('keeps session history behind the shared writer instead of raw runtime Mirrors', () => {
    let sessionSites = 0;
    for (const root of searchRoots) {
      for (const file of productionSources(join(repoRoot, root))) {
        ...
        for (
          ...
          } else if (!/schema:\s*sessionControlPlaneSchema\b/.test(options)) {
            expect(options, file).not.toMatch(/validateUpdates:\s*false\b/);
          }
        }
      }
    }
    expect(sessionSites).toBe(0);
  });
