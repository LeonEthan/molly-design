...
it('only exact authoritative application evidence settles an unknown input', async () => {
  const data = createLoroSessionData({ sessionId: sid, doc: new LoroDoc() });
  await data.commands.appendTurn({
    ...row('guide', 'user'),
  ...
  expect(await data.history.readTurn('guide')).toMatchObject({
    state: 'ready',
    turn: { status: 'handled', read: true, inputConfig: { _lodyDeliveryKind: 'steer' } },
  });
  expect(
    await data.commands.applyHistoryAction({
      kind: 'user-status',
