...
import {
  ...
  getMollyDotlodyPath,
  getMollyReposBaseDirFromDotlodyPath,
  getWorktreeHostPath,
...
describe('local project worktree paths', () => {
  ...
  it('derives session paths from the machine dotlody path', () => {
    ...
    expect(getMollyDotlodyPath('/Users/alice')).toBe('/Users/alice/.lody');
    expect(getMollyReposBaseDirFromDotlodyPath('/Users/alice/.lody/')).toBe(
  ...
  it('keeps a Windows machine path in Windows separators', () => {
    ...
    expect(getMollyDotlodyPath('C:\\Users\\alice')).toBe('C:\\Users\\alice\\.lody');
    expect(getMollyReposBaseDirFromDotlodyPath('C:\\Users\\alice\\.lody')).toBe(
  ...
  it('treats a drive-rooted forward-slash path as Windows', () => {
    // `getMollyDotlodyPath` used to emit this shape, so stored rows still carry it.
    expect(
