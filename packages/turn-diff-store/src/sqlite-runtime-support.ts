...
export function describeUnsupportedTurnDiffSqliteRuntime(
  ...
  if (!(SUPPORTED_SQLITE_ARCHS as readonly string[]).includes(runtime.arch)) {
    return (
      `@molly/turn-diff-store does not support the ${runtime.arch} architecture: ` +
  ...
  if (!Number.isFinite(napi) || napi < REQUIRED_NODE_API_VERSION) {
    return (
      `@molly/turn-diff-store needs Node-API ${REQUIRED_NODE_API_VERSION} ` +
      `(Node.js 22.14.0 through 22.x, 23.6.0+, or a later major release); received ${runtime.nodeVersion} with ` +
