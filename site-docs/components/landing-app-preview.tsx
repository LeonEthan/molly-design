...
function buildMollyPrHistory(locale: LandingLocale): SessionHistoryParsed[] {
  const isZh = locale === 'zh';
...
function buildMollyFeedbackHistory(locale: LandingLocale): SessionHistoryParsed[] {
  const isZh = locale === 'zh';
    ...
    previewAssistantMessage(
      ...
      [
        ...
        previewToolCall(
          'execute',
          isZh
            ? '运行 corepack pnpm --filter @molly/components typecheck'
            : 'Run corepack pnpm --filter @molly/components typecheck',
        ...
        ),
        {
          ...
          text: isZh
            ? [
                ...
                '',
                '验证已跑：`corepack pnpm install`、`corepack pnpm check`、`corepack pnpm --filter @molly/components typecheck`，以及 Storybook `MobileEdit` + Playwright 模拟 320px 键盘高度。',
              ].join('\n')
            : [
                ...
                '',
                'Verification: `corepack pnpm install`, `corepack pnpm check`, `corepack pnpm --filter @molly/components typecheck`, plus Storybook `MobileEdit` with Playwright simulating a 320px keyboard.',
              ].join('\n'),
        },
      ],
      t2,
  ...
  en: {
    ...
    sessions: {
      repo: {
        ...
        newTab: 'New chat',
        history: buildMollyPrHistory('en'),
      },
      localFeedback: {
        status: 'Finished working',
        ...
        newTab: 'New chat',
        history: buildMollyFeedbackHistory('en'),
      },
      fontSizePlan: {
  ...
  zh: {
    ...
    sessions: {
      repo: {
        ...
        newTab: '新对话',
        history: buildMollyPrHistory('zh'),
      },
      localFeedback: {
        status: '已完成',
        ...
        newTab: '新对话',
        history: buildMollyFeedbackHistory('zh'),
      },
      fontSizePlan: {
        status: '等待权限',
...
function buildDesignDemoHistory(
  ...
  if (turns.turn2) {
    ...
    if (turns.turn2.devDone) {
      // The real MCP tool the CLI exposes for this (molly-mcp-server.ts).
      items.push(
        previewToolCall(
          'other',
