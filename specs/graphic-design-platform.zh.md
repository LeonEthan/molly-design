...
6. 失败、取消、校验失败及未能提交如实反馈，执行状态复用既有会话显示；普通讨论或未产出设计不要求额外结果提示。终止后保留工作文件和原稿，用户显式继续时由 Agent 读取最新稿并处理草稿；重启或提交失败不自动发起模型调用。持久提交回执与历史事实不因 UI 退役而删除；已有候选内容保持可读，可作为普通文件供 Agent 处理或显式导入，不新增候选工作流。
...
- Agent 运行及产物处理期间不导入文件覆盖当前稿。回合完成后独立采集、校验和提交，成功直接显示新当前稿，无需采用确认。失败/取消保留原稿及工作文件，不因曾预览成功而自动提交。
- 没有运行回合及待完成产物处理时，用户可预览当前会话的 YAML 画稿投影，再明确点击“导入为当前稿”。导入处理未保存修改，对已看到的文档与素材快照执行结构校验及原子版本检查，成功直接保存；不先创建候选再采用。失败保留原稿、工作文件与未保存内容并报错，不自动合并；不新增作品目录导入产品，不提供遗留 `.pptd` 导入。
- 导入绑定用户已看到的确切快照，后续文件变化不能偷换内容；无法使用原快照时重新呈现。导入是明确用户动作，不伪造 Agent 读取基线；通过正式保存后才进入当前稿编辑和正式导出。重复请求沿用现有保存幂等合同。
- 预览不推进当前稿版本、不改变 Agent 草稿基线、不触发保存或完成。应用产生的画稿投影、正式存储、预览缓存和导出文件不作为创作入口监听或 Agent 新产物收集，避免同步循环；不使用“忽略下一次事件”推断来源。
