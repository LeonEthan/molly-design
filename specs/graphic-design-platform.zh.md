...
2026-09-18 预览修订：只读创作预览不再提供手动刷新或导入为当前稿入口，预览导入链退役；失败以预览作用域的错误提示如实报告。见[所属笔记](../.agents/notes/implemented/simplification/2026-09-18-preview-without-import.zh.md)；本次修订保持 draft，待补可链接的批准记录。

2026-09-18 统一画布方案：用户提出唯一实时画布、独立“保存版本”按钮及历史直切续改，并要求以固定“复刻这个设计” golden case 验证。以下纳入[完整方案](../.agents/notes/proposed/architecture/2026-09-18-version-based-canvas-editing.zh.md)的交互、保护和验收目标；用户随后授权实施，代码、回归和原生版本流程已完成。最终构建的真实复刻与同案续改已通过技术检查；驱动失败、恢复路径及视觉瑕疵保留在方案记录中，人工视觉与交互体验验收仍待完成。本修订保持 draft，历史验收不代表本轮通过。
...
第一阶段单文件字段合同为：`format: molly-canvas/1`、必填 `size`/`elements`、可选 `background`/`customFonts`/`diagnostics`/`title`。元素保留现有 `id`/`kind`/`bounds`，不采用类型区块。入口默认值、文件元数据与 canonical 的边界、显式旧 YAML 转换和测试门槛见[第一阶段审核方案](../.agents/notes/implemented/architecture/2026-09-15-single-canvas-authoring-redesign.zh.md#第一阶段审核方案)。该具体方案已通过本轮审核。
...
P1 支持宽高各 1–4096 像素的单画布。任何调整画布尺寸的实现都必须保持元素位置和大小；若调整后有元素超出画布，拒绝该调整并说明阻塞元素，不自动缩放或裁切。2026-09-16 起，编辑器内手工调整画布尺寸的入口（页面设置面板）随画布 UI Lovart 化退役，画布尺寸经 Agent 创作路径创建与调整，见[画布 Lovart UI 记录](../.agents/notes/implemented/feature/2026-09-15-canvas-lovart-ui.zh.md)。
...
- **本次统一画布最终验收**：沿用真实 K3 / Thinking High 固定用例，在最终构建中另跑独立轮次。必须观察至少两份不同非空有效构建画面，至少一份在 Agent 结束前可见；仅一次最终写入不足以证明实时效果。同案继续覆盖保存 V1、人工编辑保存 V2、选回 V1 后人工修改并让 Agent 续改存新版本、来源/历史保留、切换前保护、重启及导出；未编辑复刻图单独保留供人审。技术、实时、版本、人机续改、清理与人工视觉/交互判断全部通过才关闭本方案；已新增驱动阶段，旧验收不能代替本轮结果。细则及待验证时延目标见[完整方案](../.agents/notes/proposed/architecture/2026-09-18-version-based-canvas-editing.zh.md)。
...
- 既有参考图附件在 Molly 本地组合中的适配（P2-A3）已由 [T10](../.agents/notes/implemented/feature/2026-09-11-local-reference-attachments.zh.md)交付，复用 Lody 附件 UI、本地传输及存储，具备首条消息、设计参考快照和本地读回证据。各 Agent 的安装包图片输入仍按 [T28 矩阵](../.agents/notes/implemented/testing/2026-09-11-installed-five-agent-matrix.md)分别验收；不是新建附件系统，P3 当前画布图片操作不吸收此项。
...
2026-09-12 已确认采用[Git 设计版本方案](../.agents/notes/implemented/feature/2026-09-12-design-version-history.zh.md)：Git 为唯一历史后端，取消自建文件快照库，保留当前稿自动保存、版本检查和草稿保护；支持“存为版本”、历史选择及从旧版继续编辑。仓库由 Molly 独立管理。另已确认[人工编辑自动更新 PPTD](../.agents/notes/implemented/simplification/2026-09-12-editor-owned-pptd-save.zh.md)，并保留[先读提醒 hook](../.agents/notes/proposed/simplification/2026-09-12-noninvasive-design-hooks.zh.md)，取代旧读取同步和逐次生成证明目标。上述为当时的设计确认，后续实施证据见下一段；Spec 当时保持 draft。下面旧 hook 缺口仍是历史验收事实，不是新合同必须补 runtime 的依据。
...
2026-09-11 用户确认[整体工作流收敛](../.agents/notes/proposed/simplification/2026-09-11-design-workflow-convergence.zh.md)：最小 PPTD 语义适配、统一重新生成提交、Agent 自行处理文件冲突、直接显式导入、图像 generate/edit 与必填 model。上述为当前目标，候选旧状态仅保留历史事实；本轮只写文档和实施拆分，未实施运行时。
...
2026-09-11 [Lody 常规能力复核](../.agents/notes/proposed/simplification/2026-09-11-design-result-feedback.zh.md#lody-常规能力复核2026-09-11)更正附件判断：Lody 已有本地附件基础设施，Molly 需补组合适配；撤回“必须新建本地存储”的推断。源码和安装版菜单观察不代表完整上传/发送已验证，本轮未运行产品测试或 Agent。
