...
- 回合完成后独立采集、校验和提交，成功直接显示新当前稿，无需采用确认。失败/取消保留原稿及工作文件，不因曾预览成功而自动提交。
...
- 原保存操作改为“存为版本”，与自动保存当前稿分工。按钮等待已完成编辑的保存，捕获一致文档及确切素材/字体，成功写入 Git 历史后才显示版本已创建；首期按 V1、V2 等编号及时间展示，不强制命名。独立“另存作品”仍是创建新作品，不能只改文案冒充同一作品的版本。
- 画布右上角提供“当前稿 ▾”版本选择。选择历史版本只读查看，不修改工作文件；点击“从此编辑”才恢复为新的当前稿。若当前稿尚无对应版本，恢复前以同一 Git 后端保留“切换前”版本；保护失败不覆盖当前稿。
- 历史版本保持不变，从 V1 修改后存为新版本可得到 V4，V2/V3 仍保留；版本列表从受管理的 Git 提交/引用读取，不另存第二套权威历史。首期不引入分支树、合并或专用缩略图。
- 恢复通过既有结构/素材校验和原子版本检查，只影响所属作品；更新 Agent 可读画稿投影，保留未完成 Agent 草稿、旧回执、会话记录和无关项目文件。恢复不伪造 Agent 读取或新产物事实，也不自动调用模型。画布加载失败与磁盘保存成功分别报告。
...
选中元素后，属性栏在选区附近悬浮，保留现有类型控件及引用/图片/样式动作；按钮大小不随画稿缩放，工具栏和弹层避让可视边界。拖动时隐藏、结束后重新定位；无选区、只读及历史/源文件预览不显示可编辑栏。此项不改变单画布文档范围。验收必须包含真实 Electron 窗口中七类元素的浅色/深色选中截图及逐图视觉评估，视觉判断独立于自动化功能通过。
