...
Every managed paid operation persists its identity and authorization facts before sending, with states distinguishing prepared, dispatched, succeeded, failed, outcome_unknown. Protocol replay and reconnection only resume the same operation; unknown results are not automatically resubmitted. A new operation is created only when the user explicitly regenerates. A cancel signal does not prove the remote side stopped or that billing is avoided; late results only register as recoverable assets and do not restart an ended task.
